...
const DEPOSIT = {
	...
	BODY: {
		...
		COMPLETED: (amount, confirmation, currency) =>
			`Your ${currency.toUpperCase()} deposit for ${amount} ${currency.toUpperCase()} is confirmed and completed and it is available in your ${currency.toUpperCase()} wallet.`,
		1: (amount, currency) =>
...
const DEPOSITCANCEL = {
	TITLE: (currency, type) =>
		`${currency.toUpperCase()} ${COMMON[type.toUpperCase()]} rejected`,
	GREETING: name => COMMON.GREETING(name),
	BODY: {
...
const WITHDRAWALREQUEST = {
	TITLE: currency => `${currency.toUpperCase()} ${COMMON.WITHDRAWAL} Request`,
	GREETING: name => COMMON.GREETING(name),
	BODY: {
