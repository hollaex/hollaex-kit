...
function kit_cross_compatibility_converter() {
  ...
  if command docker ps | grep $ENVIRONMENT_EXCHANGE_NAME > /dev/null 2>&1; then 
    ...
    # Message to ask user to shutdown the exchange and start over to finish the migration flow
    echo "Warning: Your exchange is running with the files generated by HollaEx CLI v2, which requires a migration to the CLI v3 format."
    echo "Please stop and start back your exchange by running 'hollaex stop' and 'hollaex start' to continue."
