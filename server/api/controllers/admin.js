...
const getDeposits = (req, res) => {
	...
	const { user_id, currency, limit, page, order_by, order, start_date, end_date, format } = req.swagger.params;

	toolsLib.users.getUserDepositsByKitId(user_id.value, currency.value, limit.value, page.value, order_by.value, order.value, start_date.value, end_date.value, format.value)
		.then((data) => {
			if (format.value) {
...
const getWithdrawals = (req, res) => {
	...
	toolsLib.users.getUserWithdrawalsByKitId(user_id.value, currency.value, limit.value, page.value, order_by.value, order.value, start_date.value, end_date.value, format.value)
		.then((data) => {
			if (format.value) {
				if (data.data.length === 0) {
					throw new Error('No data found');
