...
const getExchangeStakes = (req, res) => {
	loggerAdmin.verbose(req.uuid, 'controllers/admin/getExchangeStakes/auth', req.auth);
	...
	if (order_by.value && typeof order_by.value !== 'string') {
		loggerAdmin.error(
			req.uuid,
			'controllers/admin/getExchangeStakes invalid order_by',
			order_by.value
		);
		return res.status(400).json({ message: 'Invalid order by' });
	}
	...
	toolsLib.user.getExchangeUserSessions({
		...
		.catch((err) => {
			loggerAdmin.error(req.uuid, 'controllers/admin/getExchangeStakes', err.message);
			return res.status(err.statusCode || 400).json({ message: errorMessageConverter(err) });
		});
};
