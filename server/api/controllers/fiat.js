...
const createWithdrawalRequestByAdmin = (req, res) => {
	loggerFiat.verbose(
		req.uuid,
		'controllers/fiat/createWithdrawalRequestByAdmin auth',
		req.auth
	);
	...
	loggerFiat.verbose(
		req.uuid,
		'controllers/fiat/createWithdrawalRequestByAdmin data',
		ip,
		amount,
		bank_id,
		currency,
	...
	toolsLib.user.getUserByKitId(userId)
		...
		.then((deposit) => {
			loggerFiat.verbose(
				req.uuid,
				'controllers/fiat/createWithdrawalRequestByAdmin then',
				'Pending mint created',
				'transaction_id:',
				deposit.transaction_id,
				'amount:',
		...
		.catch((err) => {
			loggerFiat.error(
				req.uuid,
				'controllers/fiat/createWithdrawalRequestByAdmin catch',
				err.message
			);
			return res.status(400).json({ message: err.message });
		});
...
module.exports = {
	...
	createWithdrawalRequestByAdmin
};