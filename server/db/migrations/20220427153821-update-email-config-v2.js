		...
		await queryInterface.addColumn(TABLE, COLUMN, {
			...
			defaultValue: {
				"en": {
					...
					"DOC_VERIFIED": {
						"html": "<div><p>Dear ${email} </p><p>Your uploaded KYC documents have been approved.<br>You now have access to all exchange features that require identity verification.</p><ul>${doc_information}</ul><p>To view your approved documents, visit your <a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Verification page</a></p><p> Regards<br> ${api_name} team </p></div>",
						"title": "KYC Documents Approved"
					},
					"CONFIRM_EMAIL": {
						"html": "<div><p><p>Dear ${name} </p></p><p>You have made sensitive request related to your accounts security. To verify the operation you would require to use to code below to authorize this operation.<br /><p style=@@_BIT_@@font-size: 1.2rem; text-align: center;@@_BIT_@@>${code}</p>If you did not make this request, report this immidiately and proceed to change your crendetials as soon as possible.</p><p> Regards<br> ${api_name} team </p></div>",
					...
					"SIGNUP": {
						"html": "<div> <p> Dear ${name} </p> <p> You need to confirm your email account by clicking the button below.<br> If you have any questions feel free to contact us simply by replying to this email.</p><p>Please click on the button below to proceed with your registration.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirm</Button></a></div><p> Regards<br> ${api_name} team </p></div>",
						"title": "Sign Up"
					},
					"WELCOME": {
						"html": "<div><p> Dear ${name} </p><p>Thank you for signing up to ${api_name}.</p><p>To begin trading, you must first deposit cryptocurrency or fund money to your account.Please go to your <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>account</a> and visit the <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>deposit</a> page.,</p><p>If you have any questions or concerns, please contact us simply by replying to this email.</p><p> Regards<br> ${api_name} team </p></div>",
						"title": "Welcome"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Dear ${name} </p><p>You have made a request to reset the password for your account.<br />To update your password, click on the link below.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Reset My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Regards<br> ${api_name} team </p></div>",
						"title": "Reset Password Request"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Dear ${name} </p><p>Congratulations. Your account is verified successfully.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Trade Now</Button></a></div><p> Regards<br> ${api_name} team </p></div>",
						"title": "Account Verified"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> Dear ${name} </p><p>Congratulations. Your account access level is upgraded to ${tier} tier. You will benefit from lower fees, higher withdrawal limits and other premium features.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Trade Now</Button></a></div><p> Regards<br> ${api_name} team </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Dear ${name} </p><p>You have made a ${currency} withdrawal request of ${amount} to ${address}<br /><br />Amount: ${amount}<br />Fee: ${fee} ${currency}<br />Address: ${address}<br />Network: ${network}<br /><br />In order to confirm this withdrawal, please click the button below.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirm</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Regards<br> ${api_name} team </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> Dear ${name} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Regards<br> ${api_name} team </p></div>",
						"title": "Change Password Confirmation"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> Dear ${name} </p><p> You have a new deposit for ${amount} ${currency} pending in your ${api_name} wallet. Please wait until the transaction is confirmed and your funds will be available in your wallet. Your transaction requires ${confirmation} confirmation(s) on blockchain.</p><p>Amount: ${amount} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Fee: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Regards<br> ${api_name} team </p></div>",
						"title": "${currency} Deposit pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> Dear ${name} </p><p> Your ${currency} deposit for ${amount} ${currency} is confirmed and completed and it is available in your ${currency} wallet.</p><p>Amount: ${amount} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Fee: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Regards<br> ${api_name} team </p></div>",
						"title": "${currency} Deposit completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> Dear ${name} </p><p>You made a withdrawal request for ${amount} ${currency}. Your withdrawal status is pending and will be processed shortly.</p><p>Amount: ${amount} ${currency}<br />Fee: ${fee} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
						"title": "${currency} withdrawal pending"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> Dear ${name} </p><p>Your withdrawal request for ${amount} ${currency} is processed.</p><p>Amount: ${amount} ${currency}<br />Fee: ${fee} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
				...
				"ar": {
					...
					"SIGNUP": {
						"html": "<div> <p> ${name}العزيز </p> <p> تحتاج إلى تأكيد حساب البريد الإلكتروني الخاص بك عن طريق النقر على الزر أدناه.<br> إذا كانت لديك أي أسئلة ، فلا تتردد في الاتصال بنا ببساطة عن طريق الرد على هذا البريد الإلكتروني.</p><p>الرجاء الضغط على الزر أدناه لمتابعة التسجيل الخاص بك.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>أيِّد</Button></a></div><p> تحية<br> فريق${api_name} </p></div>",
						"title": "سجّل"
					},
					"WELCOME": {
						"html": "<div><p> ${name}العزيز </p><p>شكرًا لك على الاشتراك في${api_name}.</p><p>لكي تقوم بالتداول ، يجب عليك أولاً إيداع عملة مشفرة أو تمويل الأموال في حسابك.من فضلك ، انتقل إلى <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>الحساب</a> وقم بزيارة صفحة <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>ايداع</a>.,</p><p>إذا كانت لديك أي أسئلة أو استفسارات ، فيرجى الاتصال بنا ببساطة عن طريق الرد على هذا البريد الإلكتروني.</p><p> تحية<br> فريق${api_name} </p></div>",
						"title": "أهلا بك"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> ${name}العزيز </p><p>لقد قدمت طلباً لإعادة تعيين كلمة المرور لحسابك.<br />لتحديث كلمة المرور الخاصة بك ، انقر على الرابط أدناه.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>إعادة تعيين كلمة المرور الخاصة بي</Button></a></div><p>إذا تم تقديم هذا الطلب عن طريق الخطأ ، فمن الآمن تجاهله ؛ لن يتم إجراء أي تغييرات في حسابك.</p><p>تم بدء الطلب من: ${ip}</p><p> تحية<br> فريق${api_name} </p></div>",
						"title": "طلب إعادة تعيين كلمة المرور"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> ${name}العزيز </p><p>تهانينا. تم التحقق من حسابك بنجاح.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>تداول الآن</Button></a></div><p> تحية<br> فريق${api_name} </p></div>",
						"title": "تم التحقق من الحساب"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> ${name}العزيز </p><p>تهانينا. تمت ترقية مستوى حسابك إلى مستوى${tier}. ستستفيد من رسوم أقل وحدود سحب أعلى وميزات مميزة أخرى.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>تداول الآن</Button></a></div><p> تحية<br> فريق${api_name} </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> ${name}العزيز </p><p>لقد قدّمت طلب سحب${currency} بمبلغ ${amount} الى ${address}<br /><br />المبلغ: ${amount}<br />الرسوم: ${fee} ${currency}<br />العنوان: ${address}<br />شبكة الاتصال: ${network}<br /><br />لتأكيد هذا السحب ، انقر على الزر أدناه.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>أيٍّد</Button></a></div><p>إذا تم تقديم هذا الطلب عن طريق الخطأ ، فمن الآمن تجاهله ؛ لن يتم إجراء أي تغييرات في حسابك.</p><p>تم بدء الطلب من: ${ip}</p><p> تحية<br> فريق${api_name} </p></div>",
					...
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> ${name}العزيز </p><p>لقد قدمت طلب سحب بمبلغ ${amount} ${currency} إلى عنوان. ${address}. حالة السحب الخاصة بك في قيد الإنتظار وستتم معالجتها قريبًا.</p><p>المبلغ: ${amount} ${currency}<br />الرسوم: ${fee} ${currency}<br />الحالة: ${status}<br />العنوان: ${address}<br />معرّف المعاملة: ${transaction_id}<br />شبكة الاتصال: ${network}<br /><ul>${explorers}</ul><p> تحية<br> فريق${api_name} </p></div>",
						"title": "${currency} سحب request"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> ${name}العزيز </p><p>تمت معالجة طلب سحبك بمبلغ ${amount} ${currency} و نقله إلى عنوان.${address}. </p><p>المبلغ: ${amount} ${currency}<br />الرسوم: ${fee} ${currency}<br />الحالة: ${status}<br />العنوان: ${address}<br />معرّف المعاملة: ${transaction_id}<br />شبكة الاتصال: ${network}<br /><ul>${explorers}</ul><p> تحية<br> فريق${api_name} </p></div>",
				...
				"de": {
					...
					"SIGNUP": {
						"html": "<div> <p> Sehr geehrte/r ${name} </p> <p> Sie müssen Ihr E-Mail-Konto bestätigen, indem Sie auf die unten angezeigte Taste klicken.<br> Wenn Sie Fragen haben, können Sie uns gerne kontaktieren, indem Sie einfach auf diese E-Mail antworten.</p><p>Bitte klicken Sie auf die Schaltfläche unten, um mit Ihrer Registrierung fortzufahren.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Bestätigen</Button></a></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "Sign Up"
					},
					"WELCOME": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Vielen Dank, dass Sie sich bei to ${api_name} angemeldet haben..</p><p>Um mit dem Trading zu beginnen, müssen Sie zunächst Kryptowährungen oder Geld auf Ihr Konto einzahlen.Bitte gehen Sie zu Ihrem <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Konto</a> und besuchen Sie die <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Einzahlung</a> Seite.,</p><p>Wenn Sie Fragen oder Bedenken haben, kontaktieren Sie uns bitte einfach durch Beantwortung dieser E-Mail.</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "Willkommen"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>YSie haben eine Anfrage zum Zurücksetzen des Passworts für Ihr Konto gestellt.<br />Um Ihr Passwort zu aktualisieren, klicken Sie auf den unten stehenden Link.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Mein Passwort zurücksetzen</Button></a></div><p>Wenn diese Anfrage fälschlicherweise gestellt wurde, können Sie sie getrost ignorieren; es werden keine Änderungen an Ihrem Konto vorgenommen.</p><p>Anfrage initiiert von: ${ip}</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "Passwort zurücksetzen"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Herzlichen Glückwunsch! Ihr Konto wurde erfolgreich verifiziert.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Jetzt handeln</Button></a></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Sie haben einen ${currency} Auszahlungsantrag von ${amount} an die Adresse ${address} gemacht<br /><br />Betrag: ${amount}<br />Gebühr: ${fee} ${currency}<br />Adresse: ${address}<br />Netzwerk: ${network}<br /><br />Um diese Auszahlung zu bestätigen, klicken Sie bitte auf die Taste unten.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Bestätigen</Button></a></div><p>Wenn diese Anfrage fälschlicherweise gestellt wurde, können Sie sie getrost ignorieren; es werden keine Änderungen an Ihrem Konto vorgenommen.</p><p>Anfrage initiiert von: ${ip}</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Sie haben eine Anfrage gestellt, um das Passwort für Ihr Konto zu ändern.<br />Um Ihr geändertes Passwort zu bestätigen, klicken Sie auf den unten stehenden Link.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Bestätigen Sie Mein Passwort ändern</Button></a></div><p>Wenn diese Anfrage irrtümlich gestellt wurde, können Sie sie ignorieren; Es werden keine Änderungen an Ihrem Konto vorgenommen.</p><p>Request initiated from: ${ip}</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "Passwortbestätigung ändern"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p>Sehr geehrte/r ${name} </p><p> Sie haben eine neue Einzahlung von ${amount} ${currency} in Ihrer ${api_name} Geldbörse ausstehend. Bitte warten Sie, bis die Transaktion bestätigt ist und Ihr Guthaben in Ihrer Geldbörse verfügbar ist. Ihre Transaktion erfordert ${confirmation} Bestätigung(en) in der Blockchain.</p><p>Betrag: ${amount} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Netzwerk: ${network}<br />Gebühr: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "${currency} Einzahlung pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> Sehr geehrte/r ${name} </p><p> Ihre ${currency} Anzahlung für ${amount} ${currency} ist bestätigt und abgeschlossen und steht in Ihrer ${currency} Geldbörse zur Verfügung.</p><p>Betrag: ${amount} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Netzwerk: ${network}<br />Gebühr: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "${currency} Einzahlung completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Sie haben eine Auszahlungsanforderung für ${amount} ${currency} gemacht. Ihr Auszahlungsstatus ist ausstehend und wird in Kürze bearbeitet.</p><p>Betrag: ${amount} ${currency}<br />Gebühr: ${fee} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Netzwerk: ${network}<br /><ul>${explorers}</ul><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
						"title": "${currency} Abbuchung request"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> Sehr geehrte/r ${name} </p><p>Ihr Auszahlungsantrag für ${amount} ${currency} wurde bearbeitet.</p><p>Betrag: ${amount} ${currency}<br />Gebühr: ${fee} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Netzwerk: ${network}<br /><ul>${explorers}</ul><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
				...
				"es": {
					...
					"SIGNUP": {
						"html": "<div> <p> Estimado/ Estimada ${name} </p> <p> Tiene que confirmar su cuenta de correo electrónico haciendo clic en el botón de abajo.<br> Si tiene alguna pregunta, no dude en contactarnos simplemente respondiendo a este correo electrónico.</p><p>Por favor, haga clic en el botón de abajo para proceder a su registro.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmar</Button></a></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "Registro"
					},
					"WELCOME": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Gracias por registrarse en ${api_name}.</p><p>Para poder comerciar, primero debe depositar criptomonedas en su cuenta.Por favor, vaya a su <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>cuenta</a> y visite la página de <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>depósito</a>.,</p><p>Si tiene alguna pregunta o preocupación, por favor contáctenos simplemente respondiendo a este correo electrónico.</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "Bienvenido"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Ha solicitado que se restablezca la contraseña de su cuenta.<br />Para actualizar su contraseña, haga clic en el siguiente enlace.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Restablecer mi contraseña</Button></a></div><p>Si esta solicitud se hizo por error, es seguro ignorarla; no se harán cambios en su cuenta.</p><p>Solicitud iniciada de: ${ip}</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "Solicitud de restablecimiento de la contraseña."
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Felicidades. Su cuenta ha sido verificada con éxito.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Comercie ahora</Button></a></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "Cuenta verificada"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Felicidades. El nivel de acceso a su cuenta ha sido actualizado a nivel ${tier}. Usted se beneficiará de tarifas más bajas, límites de retiro más altos y otras características premium.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Comercie ahora</Button></a></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>You have made a ${currency} withdrawal request of ${amount} to ${address}<br /><br />Cantidad: ${amount}<br />Fee: ${fee} ${currency}<br />Dirección: ${address}<br />La red: ${network}<br /><br />In order to confirm this withdrawal, please click the button below.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirm</Button></a></div><p>Si esta solicitud se hizo por error, es seguro ignorarla; no se harán cambios en su cuenta.</p><p>Solicitud iniciada de: ${ip}</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Ha realizado una solicitud para cambiar la contraseña de su cuenta.<br />Para confirmar el cambio de contraseña, haga clic en el enlace a continuación.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmar cambio de contraseña</Button></a></div><p>Si esta solicitud se realizó por error, es seguro ignorarla; no se realizarán cambios en su cuenta.</p><p>Solicitud iniciada desde: ${ip}</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "Cambiar confirmación de contraseña"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> Estimado/ Estimada ${name} </p><p>Tiene un nuevo depósito por ${amount} ${currency} pendiente en su ${api_name} billetera. Por favor espere hasta que la transacción se confirme y sus fondos estarán disponibles en su billetera. Su transacción requiere ${confirmation} confirmación(es) en la cadena de bloqueo.</p><p>Cantidad: ${amount} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />La red: ${network}<br />Tarifa: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "${currency} Depósito pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> Estimado/ Estimada ${name} </p><p> Su ${currency} depósito por ${amount} ${currency} está confirmado y completado y está disponible en su ${currency} billetera.</p><p>Cantidad: ${amount} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />La red: ${network}<br />Tarifa: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "${currency} Depósito completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Usted hizo una solicitud de retiro de ${amount} ${currency}. El estado de su retirada está pendiente y se procesará en breve.</p><p>Cantidad: ${amount} ${currency}<br />Tarifa: ${fee} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />La red: ${network}<br /><ul>${explorers}</ul><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
						"title": "${currency} Retirada request"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> Estimado/ Estimada ${name} </p><p>Su solicitud de retirada de ${amount} ${currency} se está procesando.</p><p>Cantidad: ${amount} ${currency}<br />Tarifa: ${fee} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacciónID: ${transaction_id}<br />La red: ${network}<br /><ul>${explorers}</ul><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
				...
				"fa": {
					...
					"SIGNUP": {
						"html": "<div> <p>  کاربر عزیز${name} </p> <p> شما باید ایمیل خود را با فشار دادن دکمه زیر تایید نمایید.<br> در صورت داشتن هر گونه سوال و یا ابهامی  کافیست تا آن را در پاسخ به همین ایمیل با ما درمیان بگذارید .</p><p>برای ادامه روند ثبت نام خود ، لطفا دکمه زیر را فشار دهید.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>تایید</Button></a></div><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "ثبت نام"
					},
					"WELCOME": {
					...
					"RESET_PASSWORD": {
						"html": "<div><p>  کاربر عزیز${name} </p><p>شما درخواست کلمه عبور جدید برای حساب کاربری خود را ارسال کرده اید.<br />برای بروز رسانی کلمه عبور خود از لینک زیر استفاده نمایید.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>کلمه عبور من را مجددا تنظیم کن</Button></a></div><p>اگر این درخواست را باشتباه صادر کرده اید می توانید براحتی آن را ملغی کنید بدون آنکه نگران تغییر در حساب کاربری خود باشید.</p><p>درخواست ارسالی از : ${ip}</p><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "درخواست کلمه عبور جدید"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p>  کاربر عزیز${name} </p><p>تبریک می گوییم. حساب کاربری شما تایید و ارتقا سطح داده شده است و هم اکنون می توانید براحتی فعالیت خود را آغاز نمایید.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>هم اکنون معامله کنید</Button></a></div><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "حساب کاربری تایید شد"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p>  کاربر عزیز${name} </p><p>بریک می گوییم ، سطح حساب کاربری شما به سطح ${tier}ارتقا یافت.شما می توانید از مزیت های کارمزد کمتر و سقف برداشت های بیشتر منتفع شوید.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>هم اکنون معامله کنید</Button></a></div><p> با تشکر<br> ${api_name} تیم </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p>  کاربر عزیز${name} </p><p>شما درخواست برداشت ${currency} و به مبلغ ${amount} به آدرس ${address}نموده اید.<br /><br />مبلغ: ${amount}<br />کارمزد: ${fee} ${currency}<br />آدرس: ${address}<br />شبکه: ${network}<br /><br />برای تایید برداشت خود ،دکمه زیر فشار دهید.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>تایید</Button></a></div><p>اگر این درخواست را باشتباه صادر کرده اید می توانید براحتی آن را ملغی کنید بدون آنکه نگران تغییر در حساب کاربری خود باشید.</p><p>درخواست ارسالی از : ${ip}</p><p> با تشکر<br> ${api_name} تیم </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> کاربر عزیز${name}} </p><p>شما درخواست تغییر رمز عبور حساب کاربری خود را داده اید<br />برای تایید تغییر رمز عبور، روی لینک زیر کلیک کنید.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>تغییر رمز عبور من را تأیید کنید</Button></a></div><p>اگر این درخواست به اشتباه انجام شده است، می توان آن را نادیده گرفت. هیچ تغییری در حساب شما ایجاد نخواهد شد.</p><p>درخواست از: ${ip}</p><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "Change Password Confirmation"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> کاربر عزیز${name} </p><p> شما یک واریز جدید به مبلغ ${amount} ${currency} دارید که در حال انتظار برای واریز به کیف پول ${api_name} می باشد. لطفا تا تایید تراکنش خود و مشاهده مبلغ در کیف پول خود تامل فرمایید. تراکنش شما حداقل به  ${confirmation} تایید بر روی شبکه بلاکچین نیاز دارد.` </p><p>مبلغ: ${amount} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />شبکه: ${network}<br />کارمزد: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "${currency} واریز pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> کاربر عزیز${name}} </p><p> واریز ${currency} شما به میزان ${amount} ${currency}  تکمیل و تایید شده و هم اکنون در کیف پول ${currency}  شما قابل استفاده است.</p><p>مبلغ: ${amount} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />شبکه: ${network}<br />کارمزد: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "${currency} واریز completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> کاربر عزیز${name} </p><p>شما درخواست برداشت به مبلغ ${amount} ${currency}.و به آدرس ${address} ثبت کرده اید. درخواست برداشت شما درحال انتظار بوده و به زودی مورد پردازش قرار خواهد گرفت.</p><p>مبلغ: ${amount} ${currency}<br />کارمزد: ${fee} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />شبکه: ${network}<br /><ul>${explorers}</ul><p> با تشکر<br> ${api_name} تیم </p></div>",
						"title": "${currency} برداشت request"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> کاربر عزیز${name} </p><p>درخواست برداشت شما به مبلغ ${amount} ${currency} درحال انجام و ارسال به آدرس ${address} می باشد.<br />کارمزد: ${fee} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />شبکه: ${network}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
				...
				"fr": {
					...
					"SIGNUP": {
						"html": "<div> <p> Bonjour ${name} </p> <p> Vous devez confirmer votre email en cliquant sur le bouton ci-dessous.<br> Si vous avez des questions, n'hésitez pas à nous contacter simplement en répondant à cet email.</p><p>Veuillez cliquer sur le bouton ci-dessous pour procéder à votre inscription.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmer</Button></a></div><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "S'inscrire"
					},
					"WELCOME": {
						"html": "<div><p> Bonjour ${name} </p><p>Merci pour votre inscription à ${api_name}.</p><p>Pour faire du trading, vous devez d'abord déposer de la crypto-monnaie ou verser de l'argent sur votre compte.Veuillez aller à votre <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>compte</a> et visitez la page <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>dépôt</a>.,</p><p>Si vous avez des questions ou des préoccupations, veuillez nous contacter simplement en répondant à cet email.</p><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "Bienvenue"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Bonjour ${name} </p><p>Vous avez fait une demande de réinitialisation du mot de passe de votre compte.<br />Pour mettre à jour votre mot de passe, cliquez sur le lien ci-dessous.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Réinitialiser mon mot de passe</Button></a></div><p>Si cette demande a été faite par erreur, il est prudent de l'ignorer; aucune modification ne sera apportée à votre compte.</p><p>Demande initiée depuis: ${ip}</p><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "Réinitialiser la demande de mot de passe"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Bonjour ${name} </p><p>Félicitations. Votre compte a été vérifié correctement.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Commencer le trading maintenant.</Button></a></div><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "Compte vérifié"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> Bonjour ${name} </p><p>Félicitations. Le niveau d'accès de votre compte a été mis à jour au niveau ${tier}. Vous bénéficierez de frais moins élevés, de limites de retrait plus élevées et d'autres fonctionnalités premium.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Commencer le trading maintenant.</Button></a></div><p> Bien cordialement<br> ${api_name} team </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Bonjour ${name} </p><p>Vous avez fait une demande de retrait ${currency} d'un montant de ${amount} à l'adresse suivante  ${address}<br /><br />Montant: ${amount}<br />Frais: ${fee} ${currency}<br />Addresse: ${address}<br />Réseau: ${network}<br /><br />Pour confirmer le retrait, veuillez cliquer dur le bouton ci-dessous.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmer</Button></a></div><p>Si cette demande a été faite par erreur, il est prudent de l'ignorer; aucune modification ne sera apportée à votre compte.</p><p>Demande initiée depuis: ${ip}</p><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "${currency} Retrait Demande"
					},
					"USER_VERIFICATION": {
						"html": "<div><h3>Vérification de l'utilisateur</h3><div>Utilisateur @@_BIT_@@${email}@@_BIT_@@ a téléchargé ses documents pour vérification. Veuillez vérifier ses documents.</div></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> Bonjour ${name} </p><p>Vous avez fait une demande de modification du mot de passe de votre compte.<br />Pour confirmer la modification de votre mot de passe, cliquez sur le lien ci-dessous.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmer le changement de mon mot de passe</Button></a></div><p>Si cette demande a été faite par erreur, vous pouvez l'ignorer; aucune modification ne sera apportée à votre compte.</p><p>Demande initiée par: ${ip}</p><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "Change Password Confirmation"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> Bonjour ${name} </p><p> Vous avez un nouveau dépôt pour ${amount} ${currency} en attente dans votre portefeuille ${api_name} . Veuillez attendre que la transaction soit confirmée et vos fonds seront disponible dans votre portefeuille. Votre transaction nécessite ${confirmation} confirmation(s) sur la blockchain.</p><p>Montant: ${amount} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Réseau: ${network}<br />Frais: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "${currency} Deposit pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> Bonjour ${name} </p><p> Votre ${currency} dépôt pour ${amount} ${currency} est confirmé et complété et il est disponible dans votre portefeuille ${currency}.</p><p>Montant: ${amount} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Réseau: ${network}<br />Frais: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "${currency} Deposit completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> Bonjour ${name} </p><p>Vous avez fait une demande de retrait de ${amount} ${currency}. Le statut de votre retrait est en attente et sera traité sous peu.</p><p>Montant: ${amount} ${currency}<br />Frais: ${fee} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Réseau: ${network}<br /><ul>${explorers}</ul><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "${currency} withdrawal pending"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> Bonjour ${name} </p><p>Votre demande de retrait de ${amount} ${currency} est traité.</p><p>Montant: ${amount} ${currency}<br />Frais: ${fee} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Réseau: ${network}<br /><ul>${explorers}</ul><p> Bien cordialement<br> ${api_name} team </p></div>",
						"title": "${currency} withdrawal completed"
					}
				},
				"id": {
					...
					"SIGNUP": {
						"html": "<div> <p> Kepada ${name} </p> <p> Konfirmasikan akun email Anda dengan klik Anda dengan klik tombol di bawah.<br> Jika Anda memiliki pertanyaan, jangan ragu untuk menghubungi kami dengan membalas email ini.</p><p>Silakan klik tombol di bawah untuk melanjutkan pendaftaran Anda.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Konfirmasi</Button></a></div><p> Salam<br> ${api_name} tim </p></div>",
						"title": "Daftar"
					},
					"WELCOME": {
						"html": "<div><p> Kepada ${name} </p><p>Terima kasih telah mendaftar di ${api_name}.</p><p>Untuk memulai perdagangan, Anda perlu deposito cryptocurrency atau dana ke akun Anda terlebih dahulu. Silakan pergi ke <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>akun</a>Anda dan kunjungi halaman <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Deposito</a>.,</p><p>Jika Anda memiliki pertanyaan atau kekhawatiran, jangan ragu untuk menghubungi kami dengan membalas email ini.</p><p> Salam<br> ${api_name} tim </p></div>",
						"title": "Selamat datang"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Kepada ${name} </p><p>Anda telah meminta reset kata sandi akun Anda.<br />Untuk memperbarui kata sandi Anda, silakan klik tautan di bawah.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Reset Kata Sandi Saya</Button></a></div><p>Jika Anda tidak ingin melanjutkan permintaan ini, silakan abaikan ini; tidak ada perubahan pada akun Anda.</p><p>Permintaan dari: ${ip}</p><p> Salam<br> ${api_name} tim </p></div>",
						"title": "Permintaan Reset Kata Sandi"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Kepada ${name} </p><p>Selamat. Akun Anda telah berhasil terverifikasi.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Berdagang Sekarang</Button></a></div><p> Salam<br> ${api_name} tim </p></div>",
						"title": "Akun Terverifikasi"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> Kepada ${name} </p><p>Selamat. Tingkat akses akun Anda telah diperbarui ke tingkat ${tier}. Anda akan mendapatkan keuntungan dari biaya lebih rendah, batas penarikan lebih tinggi dan fitur premium lainnya.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Berdagang Sekarang</Button></a></div><p> Salam<br> ${api_name} tim </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Kepada ${name} </p><p>Anda telah membuat permintaan penarikan ${currency} dengan jumlah ${amount} ke ${address}<br /><br />Jumlah: ${amount}<br />Biaya: ${fee} ${currency}<br />Alamat: ${address}<br />Jaringan: ${network}<br /><br />Untuk konfirmasi penarikan ini, silakan klik tombol di bawah.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Konfirmasi</Button></a></div><p>Jika Anda tidak ingin melanjutkan permintaan ini, silakan abaikan ini; tidak ada perubahan pada akun Anda.</p><p>Permintaan dari: ${ip}</p><p> Salam<br> ${api_name} tim </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> Kepada ${name} </p><p>Anda telah membuat permintaan untuk mengubah kata sandi akun Anda.<br />Untuk mengonfirmasi kata sandi Anda diubah, klik tautan di bawah<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Konfirmasi Ubah Kata Sandi Saya</Button></a></div><p>Jika permintaan ini dibuat karena kesalahan, Anda dapat mengabaikannya; tidak ada perubahan yang akan dilakukan pada akun</p><p>Permintaan dimulai dari: ${ip}</p><p> Salam<br> ${api_name} tim </p></div>",
						"title": "Ubah Konfirmasi Kata Sandi"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> Kepada ${name} </p><p> Ada deposit baru untuk ${amount} ${currency} yang sedang dalam proses di dompet ${api_name} Anda. Mohon ditunggu sampai transaksi dikonfirmasi dan dana Anda akan tersedia di dompet Anda. Please wait until the transaction is confirmed and your funds will be available in your wallet. Transaksi Anda memerlukan ${confirmation} konfirmasi dalam blockchain.</p><p>Jumlah: ${amount} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Jaringan: ${network}<br />Biaya: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Salam<br> ${api_name} tim </p></div>",
						"title": "${currency} Deposito pending"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> Kepada ${name} </p><p> Deposit ${currency} Anda untukr ${amount} ${currency} telah dikonfirmasi dan berhasil dibuat dan tersedia di dompet ${currency} Anda.</p><p>Jumlah: ${amount} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Jaringan: ${network}<br />Biaya: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Salam<br> ${api_name} tim </p></div>",
						"title": "${currency} Deposito completed"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> Kepada ${name} </p><p>Anda telah membuat permintaan penarikan untuk ${amount} ${currency}. ke alamat ${address}. Penarikan Anda sedang dalam proses dan akan segera diproses.</p><p>Jumlah: ${amount} ${currency}<br />Biaya: ${fee} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Jaringan: ${network}<br /><ul>${explorers}</ul><p> Salam<br> ${api_name} tim </p></div>",
						"title": "${currency} Penarikan pending"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p>Kepada ${name} </p><p>Permintaan penarikan Anda untuk ${amount} ${currency} telah diproses dan ditransfer ke alamat ${address}.</p><p>Jumlah: ${amount} ${currency}<br />Biaya: ${fee} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Jaringan: ${network}<br /><ul>${explorers}</ul><p> Salam<br> ${api_name} tim </p></div>",
				...
				"ja": {
					...
					"SIGNUP": {
						"html": "<div> <p> ${name}様 </p> <p>下のボタンをクリックしてメールアカウントを確認する必要があります。<br> ご不明な点がございましたら、このメールに返信するだけでお気軽にお問い合わせください。</p><p>ご不明な点がございましたら、このメールに返信するだけでお気軽にお問い合わせください。</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>出金承認</Button></a></div><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "会員登録"
					},
					"WELCOME": {
						"html": "<div><p> ${name}様 </p><p>${api_name}をご利用いただき誠にありがとうございます。.</p><p>取引を始めるため、まず仮想通貨、または資金をアカウントに入金する必要があります。お客様の <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>アカウント</a>に移動して、 <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>入金</a> ページを訪問してください。`.,</p><p>お問い合わせは、このメールにてご返信ください。</p><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "ようこそ"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> ${name}様 </p><p>お客様のアカウントから、パスワードの再設定がリクエストされました。<br />下のリンクをクリックして、パスワードのアップデートを続行してください。<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>パスワード再設定</Button></a></div><p>このリクエストの続行を望まない場合、このメールは無視してください。お客様のアカウントに変更は一切適用されません。</p><p>リクエスト元： ${ip}</p><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "パスワード再設定のリクエスト"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> ${name}様 </p><p>おめでとうございます。お客様のアカウントの認証が完了しました。</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>取引を始める</Button></a></div><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "アカウントの認証完了"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> ${name}様 </p><p>おめでとうございます。お客様のアカウントのレベルが、${tier}にアップグレードされました。これから、より安い手数料や高い出金限度額など、様々なプレミアム特典をご利用になれます。</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>取引を始める</Button></a></div><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "Account Upgraded"
					},
					"DEPOSIT_CANCEL": {
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> ${name}様 </p><p>お客様のアカウントから${address}へ${amount} ${currency}出金がリクエストされました。<br /><br />金額： ${amount}<br />手数料： ${fee} ${currency}<br />アドレス： ${address}<br />ネットワーク: ${network}<br /><br />出金を承認するためには、下のボタンをクリックしてください。<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>出金承認</Button></a></div><p>このリクエストの続行を望まない場合、このメールは無視してください。お客様のアカウントに変更は一切適用されません。</p><p>リクエスト元： ${ip}</p><p> 敬具<br> ${api_name} チーム </p></div>",
					...
					"BANK_VERIFIED": {
						"html": "<div><p> ${name}様 </p><p>保留中の銀行口座が確認されました。 これで、有効なアカウントを銀行口座を必要とする交換操作に使用できます。</p><div><strong>確認済みの銀行口座:</strong>${list_detail_bank_account}</div><p><a href=@@_BIT_@@${link_verification}@@_BIT_@@>現在の銀行口座を表示するには、取引所の[確認]タブにアクセスしてください</a></p><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "銀行確認済み"
					},
					"USER_ID_VERIFICATION_REJECT": {
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> ${name}様 </p><p>アカウントのパスワードの変更をリクエストしました。<br />パスワードが変更されたことを確認するには、以下のリンクをクリックしてください。<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>パスワードの変更を確認する</Button></a></div><p>この要求が誤って行われた場合は、無視しても問題ありません。 アカウントに変更は加えられません。</p><p>リクエストの開始元： ${ip}</p><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "パスワードの変更の確認"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> ${name}様 </p><p> 現在、お客様の${api_name}ウォレットへの${amount} ${currency}入金は保留中です。取引が承認されるまでお待ちください お客様の取引には、ブロックチェーン上で${confirmation}個の承認が必要です。</p><p>金額: ${amount} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />ネットワーク: ${network}<br />手数料: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "${currency}デポジットは保留中です"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> ${name}様 </p><p> お客様の${amount} ${currency}入金が完了しました。お客様の${currency}ウォレットから確認および利用が可能です。 </p><p>金額: ${amount} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />ネットワーク: ${network}<br />手数料: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "${currency}デポジットが完了しました"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> ${name}様 </p><p>お客様のアドレス${address}に${amount} ${currency}出金がリクエストされました。現在の取引状態は保留中ですが、まもなく完了する予定です。</p><p>金額: ${amount} ${currency}<br />手数料: ${fee} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />ネットワーク: ${network}<br /><ul>${explorers}</ul><p> 敬具<br> ${api_name} チーム </p></div>",
						"title": "${currency}の引き出しは保留中"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> ${name}様 </p><p>お客様の${amount} ${currency}出金が完了し、アドレス${address}に振り込みされました。</p><p>金額: ${amount} ${currency}<br />手数料: ${fee} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />ネットワーク: ${network}<br /><ul>${explorers}</ul><p> 敬具<br> ${api_name} チーム </p></div>",
				...
				"ko": {
					...
					"SIGNUP": {
						"html": "<div> <p> ${name}님 </p> <p> 안녕하세요. 아래 버튼을 클릭하여 회원님의 이메일 계정을 인증해주시기바랍니다. <br> 문의사항은 본 메일에 회신하여 문의하실 수 있습니다.</p><p>아래 버튼을 클릭하여 등록 절차를 진행하시기 바랍니다.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>나의 계정 활성화</Button></a></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "회원가입"
					},
					"WELCOME": {
						"html": "<div><p> ${name}님 </p><p>누구나 빠르고 쉬운 거래가 가능한 가상화폐거래소 ${api_name}를 이용해주셔서 감사합니다.</p><p>거래를 시작하기 위해선 먼저, 비트코인 또는 현금을 계좌에 입금하여야 합니다. <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>계정</a> 페이지로 이동하여 <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>입금</a> 페이지를 방문해주시기 바랍니다.,</p><p>문의사항은 이메일로 문의해주시기 바랍니다.</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "환영합니다"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> ${name}님 </p><p>회원님의 계정에 대한 비밀번호 재설정을 요청하셨습니다.<br />아래 링크를 클릭하여 비밀번호 업데이트를 진행하시기 바랍니다.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>비밀번호 재설정</Button></a></div><p>만약 이 요청을 원하지 않는다면, 회원님의 계정에는 변경사항이 없으니 본 메일을 무시해 주시기 바랍니다.</p><p>요청하신곳: ${ip}</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "비밀번호 재설정 요청"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> ${name}님 </p><p>축하합니다. 회원님의 신원이 확인되어 계정이 승급되었습니다. 이제 거래를 시작하기 위한 모든 준비가 되었습니다.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>거래하기</Button></a></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "계정 인증완료"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> ${name}님 </p><p>축하합니다. 회원님의 계정이 레벨 ${tier} 로 업그레이드 되었습니다. 이제 더 낮은 수수료와 높아진 출금한도를 비롯한 다른 프리미엄 혜택을 받으실 수 있습니다.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>거래하기</Button></a></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "계정 업그레이드완료"
					},
					"DEPOSIT_CANCEL": {
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> ${name}님 </p><p>회원님은 ${address} 로 ${amount} 의 ${currency}을 출금요청하였습니다.<br /><br />금액: ${amount}<br />수수료: ${fee} ${currency}<br />주소: ${address}<br />회로망: ${network}<br /><br />출금 요청을 완료하시려면 아래버튼을 클릭해주시기 바랍니다.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>나의 계정 활성화</Button></a></div><p>만약 이 요청을 원하지 않는다면, 회원님의 계정에는 변경사항이 없으니 본 메일을 무시해 주시기 바랍니다.</p><p>요청하신곳: ${ip}</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
					...
					"BANK_VERIFIED": {
						"html": "<div><p> ${name}님 </p><p>보류 중인 은행 계좌가 확인되었습니다. 이제 유효한 계정을 은행 계좌가 필요한 교환 작업에 사용할 수 있습니다.</p><div><strong>확인된 은행 계좌:</strong>${list_detail_bank_account}</div><p><a href=@@_BIT_@@${link_verification}@@_BIT_@@>현재 은행 계좌를 보려면 거래소의 확인 탭을 방문하세요.</a></p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "은행 인증"
					},
					"USER_ID_VERIFICATION_REJECT": {
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> ${name}님 </p><p>계정의 비밀번호 변경을 요청하셨습니다.<br />비밀번호 변경을 확인하려면 아래 링크를 클릭하세요.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>내 비밀번호 변경 확인</Button></a></div><p>이 요청이 오류로 이루어진 경우 무시해도 됩니다. 귀하의 계정은 변경되지 않습니다.</p><p>요청 시작: ${ip}</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "비밀번호 변경 확인"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> ${name}님 </p><p> 회원님의 ${api_name} 지갑으로 ${amount} ${currency} 입금이 진행 중입니다. 거래가 승인되고 지갑에 자금이 입금될 때까지 기다려주십시오. 회원님의 거래에는 비트코인 블록체인 상 ${confirmation} 개의 승인이 요구됩니다. </p><p>금액: ${amount} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />회로망: ${network}<br />수수료: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "${currency} 입금 보류 중"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> ${name}님} </p><p> 회원님의 ${amount} ${currency} 입금이 완료되었습니다. 회원님의 ${currency} 지갑에서 확인 및 이용하실 수 있습니다.</p><p>금액: ${amount} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />회로망: ${network}<br />수수료: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "${currency} 입금 완료"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> ${name}님 </p><p>회원님의 ${amount} ${currency} 출금이 요청되었습니다. 출금 대기 중이며, 곧 완료될 예정입니다.</p><p>금액: ${amount} ${currency}<br />수수료: ${fee} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />회로망: ${network}<br /><ul>${explorers}</ul><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
						"title": "${currency} 출금 보류 중"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> ${name}님 </p><p>회원님의 ${amount} ${currency}를 출금이 완료되어 회원님의 계좌로 이체되었습니다.</p><p>금액: ${amount} ${currency}<br />수수료: ${fee} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />회로망: ${network}<br /><ul>${explorers}</ul><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
				...
				"pt": {
					...
					"SIGNUP": {
						"html": "<div> <p> Dear ${name} </p> <p> Você precisa confirmar sua conta de e-mail clicando no botão abaixo.<br> Se você tiver alguma dúvida, basta entrar em contato através deste e-mail.</p><p>Por favor, clique no botão abaixo para prosseguir.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmar</Button></a></div><p> Saudações<br> ${api_name} equipe </p></div>",
						"title": "Registrar"
					},
					"WELCOME": {
						"html": "<div><p> Dear ${name} </p><p>Obrigado por se registrar-se no ${api_name}.</p><p>Para começar a fazer trade, você deve primeiro fazer um depósito em criptomoedas ou dinheiro em sua conta.Vá para sua <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>conta</a> e visite a página de <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>depósito</a>.,</p><p>Se você tiver alguma dúvida ou questão, basta entrar em contato conosco através deste e-mail.</p><p> Saudações<br> ${api_name} equipe </p></div>",
						"title": "Bem-vindo(a)"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> Dear ${name} </p><p>Você solicitou a redefinição da senha de sua conta.<br />Para atualizar sua senha, clique no link abaixo.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Criar nova senha</Button></a></div><p>Se esta solicitação foi feita por engano, gentileza ignorá-la; nenhuma alteração será feita em sua conta.</p><p>Solicitação iniciada em: ${ip}</p><p> Saudações<br> ${api_name} equipe </p></div>",
						"title": "Solicitação para redefinir a senha"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> Dear ${name} </p><p>Parabéns. Sua conta foi verificada com sucesso.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Comece a fazer trade agora!</Button></a></div><p> Saudações<br> ${api_name} equipe </p></div>",
						"title": "Conta verificada"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> Dear ${name} </p><p>Parabéns! O nível de acesso da sua conta recebeu um upgrade e foi para o nível ${tier}. Você terá como benefícios taxas mais baixas, limites de saque mais altos e outros recursos premium.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Comece a fazer trade agora!</Button></a></div><p> Saudações<br> ${api_name} equipe </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> Dear ${name} </p><p>Você fez uma solicitação de saque de ${currency} de ${amount} para ${address}<br /><br />Quantia: ${amount}<br />Taxa: ${fee} ${currency}<br />Endereço: ${address}<br />Rede: ${network}<br /><br />Para confirmar este saque, por favor clique no botão abaixo.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Confirmar</Button></a></div><p>Se esta solicitação foi feita por engano, gentileza ignorá-la; nenhuma alteração será feita em sua conta.</p><p>Solicitação iniciada em: ${ip}</p><p> Saudações<br> ${api_name} equipe </p></div>",
				...
				"vi": {
					...
					"SIGNUP": {
						"html": "<div> <p> ${name} thân mến </p> <p> Quý khách cần xác nhận địa chỉ email bằng cách nhấn vào nút bên dưới.<br> Nếu có bất kỳ thắc mắc nào, đừng ngần ngại liên hệ với chúng tôi bằng cách hồi đáp lại thư này.</p><p>Vui lòng nhấn vào nút bên dưới để hoàn tất thủ tục đăng ký.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Xác nhận</Button></a></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "Đăng ký"
					},
					"WELCOME": {
						"html": "<div><p> ${name} thân mến </p><p>Cảm ơn quý khách đã đăng ký thành viên tại ${api_name}.</p><p>Để giao dịch, trước tiên quý khách cần nạp tiền điện tử hoặc tiền mặt vào tài khoản.Vui lòng truy cập vào <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Tài khoản</a> và chuyển tới mục <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>Nạp tiền</a>.,</p><p>Hãy hồi đáp lại thư này này nếu quý khách có bất kỳ câu hỏi hoặc mối bận tâm nào.</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "Chào mừng quý khách"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> ${name} thân mến </p><p>Quý khách vừa thực hiện yêu cầu đặt lại mật khẩu cho tài khoản của mình.<br />Để cập nhật mật khẩu, vui lòng nhấn vào liên kết phía dưới.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Đặt lại mật khẩu</Button></a></div><p>Trong trường hợp không mong muốn thay đổi nêu trên, hãy bỏ qua tin nhắn này. Chúng tôi sẽ không thực hiện bất kỳ thay đổi nào trên tài khoản của quý khách.</p><p>Yêu cầu được gửi đến từ: ${ip}</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "Yêu cầu đặt lại mật khẩu"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> ${name} thân mến </p><p>Chúc mừng. Tài khoản của quý khách đã được xác thực thành công.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Giao dịch ngay bây giờ</Button></a></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "Tài khoản đã được xác thực"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> ${name} thân mến </p><p>Chúc mừng. Tài khoản của quý khách đã được nâng cấp lên cấp bậc ${tier}. Quý khách sẽ nhận được nhiều ưu đãi như phí giao dịch rẻ hơn, hạn mức rút tiền cao hơn và các phần thưởng khác.</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Giao dịch ngay bây giờ</Button></a></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> ${name} thân mến </p><p>Quý khách vừa thực hiện yêu cầu rút ${currency} với số lượng ${amount} tới ${address}<br /><br />Số lượng: ${amount}<br />Phí: ${fee} ${currency}<br />Địa chỉ: ${address}<br />Mạng: ${network}<br /><br />Để xác nhận yêu cầu rút tiền này, vui lòng nhấn vào nút bên dưới.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Xác nhận</Button></a></div><p>Trong trường hợp không mong muốn thay đổi nêu trên, hãy bỏ qua tin nhắn này. Chúng tôi sẽ không thực hiện bất kỳ thay đổi nào trên tài khoản của quý khách.</p><p>Yêu cầu được gửi đến từ: ${ip}</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
					...
					"CHANGE_PASSWORD": {
						"html": "<div><p> ${name} thân mến </p><p>Bạn đã yêu cầu thay đổi mật khẩu cho tài khoản của mình.<br />Để xác nhận mật khẩu của bạn đã thay đổi, hãy nhấp vào liên kết bên dưới.<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>Xác nhận thay đổi mật khẩu của tôi</Button></a></div><p>Nếu yêu cầu này được đưa ra do nhầm lẫn, bạn có thể yên tâm bỏ qua nó; không có thay đổi nào sẽ được thực hiện đối với tài khoản của bạn.</p><p>Yêu cầu bắt đầu từ: ${ip}</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "Thay đổi xác nhận mật khẩu"
					},
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> ${name} thân mến </p><p> Một giao dịch nạp ${amount} ${currency} vào địa chỉ ví ${api_name} của quý khách đang chờ được xử lý. Vui lòng chờ đợi cho đến khi giao dịch được xác nhận và tiền được nạp vào ví của quý khách. Yêu cầu giao dịch của quý khách cần nhận được ${confirmation} kết quả đồng thuận trên blockchain.</p><p>Số lượng: ${amount} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Mạng: ${network}<br />Phí: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "${currency} Nạp tiền chưa giải quyết"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> ${name} thân mến </p><p> Giao dịch nạp ${currency} với số lượng ${amount} ${currency} đã được xác nhận và hoàn tất thành công. Số tiền đã được cập nhập vào ví ${currency} của quý khách.</p><p>Số lượng: ${amount} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Mạng: ${network}<br />Phí: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "${currency} Nạp tiền hoàn thành"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> ${name} thân mến </p><p>Quý khách vừa thực hiện yêu cầu rút ${amount} ${currency} tới địa chỉ ${address}. Yêu cầu rút tiền của quý khách đang ở trạng thái chờ xử lý và sẽ sớm được thực hiện.</p><p>Số lượng: ${amount} ${currency}<br />Phí: ${fee} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Mạng: ${network}<br /><ul>${explorers}</ul<p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
						"title": "${currency} Rút tiền chưa giải quyết"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> ${name} thân mến </p><p>Yêu cầu rút ${amount} ${currency} của quý khách đã được thực hiện và số tiền tương ứng đã được chuyển thành công tới địa chỉ ${address}.</p><p>Số lượng: ${amount} ${currency}<br />Phí: ${fee} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Mạng: ${network}<br /><ul>${explorers}</ul><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
				...
				"zh": {
					...
					"SIGNUP": {
						"html": "<div> <p> ${name} </p> <p>需要通过点击下面的按钮确认你的邮箱地址。<br> 如果你有任何问题，请随时联系我们，只需回复此邮件即可。</p><p>请点击下方按钮，进行注册</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>确认</Button></a></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "注册会员"
					},
					"WELCOME": {
						"html": "<div><p> ${name} </p><p>感谢你在 ${api_name} 进行注册.</p><p>若要进行交易，必须先将加密货币或资金存入你的账户。.前往 <a href=@@_BIT_@@${link_account}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>账号</a> 并访问 <a href=@@_BIT_@@${link_deposit}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@>充值</a> 页面。.,</p><p>如果您有任何问题，请回复此邮件与我们进行联系。</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "注册会员"
					},
					"RESET_PASSWORD": {
						"html": "<div><p> ${name} </p><p>你已申请重置密码<br />请点击下方链接，进行密码更新<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@重置我的密码</Button></a></div><p>如果该请求是错误的，请忽略，并且你的账号不会有任何改变</p><p>提出申请的出处: ${ip}</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "申请重置密码"
					},
					"ACCOUNT_VERIFY": {
						"html": "<div><p> ${name} </p><p>恭喜你，你的账号验证成功。</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>进行交易</Button></a></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "账号认证完成"
					},
					"ACCOUNT_UPGRADE": {
						"html": "<div><p> ${name} </p><p>恭喜你，你的账号级别已升级至${tier}，你将可以享受更低的费用、更高的提款限额和其他高级功能。</p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>进行交易</Button></a></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "账号升级完成"
					},
					"DEPOSIT_CANCEL": {
					...
					"WITHDRAWAL_REQUEST": {
						"html": "<div><p> ${name} </p><p>你已提出${currency}提款申请，金额为${amount}至${address}。<br /><br />金额: ${amount}<br />手续费: ${fee} ${currency}<br />地址: ${address}<br />网络: ${network}<br /><br />为完成此次提款申请，请点击下方按钮。<br /></p><div style=@@_BIT_@@padding-top: 10px; margin-bottom: 10px;@@_BIT_@@><a href=@@_BIT_@@${link}@@_BIT_@@ target=@@_BIT_@@_blank@@_BIT_@@><Button style=@@_BIT_@@cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;@@_BIT_@@>确认</Button></a></div><p>如果该请求是错误的，请忽略，并且你的账号不会有任何改变</p><p>提出申请的出处: ${ip}</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
					...
					"BANK_VERIFIED": {
						"html": "<div><p> ${name} </p><p>一個待處理的銀行賬戶已經過驗證。 您的有效賬戶現在可用於需要銀行賬戶的兌換操作。</p><div><strong>已驗證的銀行賬戶:</strong>${list_detail_bank_account}</div><p><a href=@@_BIT_@@${link_verification}@@_BIT_@@>要查看您當前的銀行賬戶，請訪問交易所的驗證選項卡</a></p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "銀行認證"
					},
					"USER_ID_VERIFICATION_REJECT": {
					...
					"DEPOSIT_PENDING": {
						"html": "<div><div><p> ${name} </p><p> 你的${api_name}钱包中有一笔${amount} ${currency}的新存款正在等待处理，待交易被确认后，资金才会进入到你的钱包中，该交易需要在区块链上进行${confirmation}确认。</p><p>金额: ${amount} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />网络: ${network}<br />手续费: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "${currency} 存款待處理"
					},
					"DEPOSIT_COMPLETED": {
						"html": "<div><div><p> ${name}  </p><p> 你的${amount} ${currency}存款已完成。你的${currency}可在钱包中进行确认。</p><p>金额: ${amount} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />网络: ${network}<br />手续费: ${fee} ${currency}<br /><ul>${explorers}</ul></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "${currency} 存款完成"
					},
					"WITHDRAWAL_PENDING": {
						"html": "<div><p> ${name} </p><p>你向该地址${address}申请了${amount} ${currency}的提款请求，该提款地址正在等待处理并会很快得到处理。</p><p>金额: ${amount} ${currency}<br />手续费: ${fee} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />网络: ${network}<br /><ul>${explorers}</ul><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "${currency} 提款待處理"
					},
					"WITHDRAWAL_COMPLETED": {
						"html": "<div><p> ${name} </p><p>你的${amount} ${currency}已向该地址${address}提款完毕。</p><p>金额: ${amount} ${currency}<br />手续费: ${fee} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />网络: ${network}<br /><ul>${explorers}</ul><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
						"title": "${currency} 提款完成"
					}
				}
			}
