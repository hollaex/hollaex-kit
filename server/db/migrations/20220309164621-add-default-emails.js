...
module.exports = {
	up: (queryInterface, Sequelize) => {
		return queryInterface.bulkUpdate(TABLE, { email: JSON.stringify(emails)});
	},

	down: (queryInterface, Sequelize) => {
		return queryInterface.bulkUpdate(TABLE, { email: '{}' });
	}
};
...
const emails =  {
	"en": {
		...
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> Dear ${name} </p><p> You have a new deposit for ${amount} ${currency} pending in your ${api_name} wallet. Please wait until the transaction is confirmed and your funds will be available in your wallet. Your transaction requires ${confirmation} confirmation(s) on blockchain.</p><p>Amount: ${amount} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Fee: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Regards<br> ${api_name} team </p></div>",
			"title": "${currency} Deposit pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> Dear ${name} </p><p> Your ${currency} deposit for ${amount} ${currency} is confirmed and completed and it is available in your ${currency} wallet.</p><p>Amount: ${amount} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Fee: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Regards<br> ${api_name} team </p></div>",
			"title": "${currency} Deposit completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> Dear ${name} </p><p>You made a withdrawal request for ${amount} ${currency}. Your withdrawal status is pending and will be processed shortly.</p><p>Amount: ${amount} ${currency}<br />Fee: ${fee} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
			"title": "${currency} withdrawal pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> Dear ${name} </p><p>Your withdrawal request for ${amount} ${currency} is processed.</p><p>Amount: ${amount} ${currency}<br />Fee: ${fee} ${currency}<br />Status: ${status}<br />Address: ${address}<br />Transaction ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
	...
	"ar": {
		...
		"DEPOSIT_CANCEL": {
			"html": "<div><p> ${name}العزيز </p><p>We were not able to find or process your ${currency} deposit made on ${date} for ${amount}. Thus, the transaction is rejected by our system.</p><p>If you have any further inquiries, you can reply to this email</p><p>معرّف المعاملة:${txid} <br />المبلغ: ${amount}<br />Status: Rejected</p><p> تحية<br> فريق${api_name} </p></div>",
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> ${name}العزيز </p><p>لقد قدّمت طلب سحب${currency} بمبلغ ${amount} الى ${address}<br /><br />المبلغ: ${amount}<br />الرسوم: ${fee} ${currency}<br />العنوان: ${address}<br />Network: ${network}<br /><br />لتأكيد هذا السحب ، انقر على الزر أدناه.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">أيٍّد</Button></a></div><p>إذا تم تقديم هذا الطلب عن طريق الخطأ ، فمن الآمن تجاهله ؛ لن يتم إجراء أي تغييرات في حسابك.</p><p>تم بدء الطلب من: ${ip}</p><p> تحية<br> فريق${api_name} </p></div>",
		...
		"INVALID_ADDRESS": {
			"html": "<div><p> ${name}العزيز </p><p>Your ${currency} withdrawal for ${amount} was being sent to an invalid address and is rejected.</p><p>Address: ${address}</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Invalid Withdrawal Address"
		},
		"USER_DEACTIVATED": {
			"html": "<div><p>Your account ${email} has been deactivated. You will not be able to use your account until it is activated by the exchange admin.</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Account ${type}"
		},
		"USER_ACTIVATED": {
			"html": "<div><p>Your account ${email} has been activated. You are now able to use your account.</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Account ${type}"
		},
		"DISCOUNT_UPDATE": {
			"html": "<div><p> ${name}العزيز </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> ${name}العزيز </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
			"html": "<div><p> ${name}العزيز </p><p>Your recent ID verification is processed and is unfortunately rejected. For further actions read the message from our expert below:</p><p>Message: ${message}</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "ID Verification Rejected"
		},
		"USER_BANK_VERIFICATION_REJECT": {
			"html": "<div><p> ${name}العزيز </p><p>Your new bank registration is processed and is unfortunately rejected. For further actions read the message from our expert below:</p><p>Message: ${message}</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "New Bank Application Rejected"
		},
		"PASSWORD_CHANGED": {
			"html": "<div><p> ${name}العزيز </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> ${name}العزيز </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>تم بدء الطلب من: ${ip}</p><p> تحية<br> فريق${api_name} </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> ${name}العزيز </p><p> ${amount} لديك إيداع جديد بمبلغ ${currency} في قيد الانتظار في محفظتك ${api_name} .يرجى الانتظار حتى يتم تأكيد المعاملة وستتوفر أموالك في محفظتك. تتطلب معاملتك. ${confirmation} تأكيداً (تأكيداتٍ) على البلوك تشين.</p><p>المبلغ: ${amount} ${currency}<br />الحالة: ${status}<br />العنوان: ${address}<br />معرّف المعاملة: ${transaction_id}<br />Network: ${network}<br />الرسوم: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> تحية<br> فريق${api_name} </p></div>",
			"title": "${currency} ايداع pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> ${name}العزيز </p><p> Yايداعك لـ ${currency} ${amount} ${currency} قد تم تأكيده و تكميله و موجود في ${currency} محفظتك.</p><p>المبلغ: ${amount} ${currency}<br />الحالة: ${status}<br />العنوان: ${address}<br />معرّف المعاملة: ${transaction_id}<br />Network: ${network}<br />الرسوم: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> تحية<br> فريق${api_name} </p></div>",
			"title": "${currency} ايداع completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> ${name}العزيز </p><p>لقد قدمت طلب سحب بمبلغ ${amount} ${currency} إلى عنوان. ${address}. حالة السحب الخاصة بك في قيد الإنتظار وستتم معالجتها قريبًا.</p><p>المبلغ: ${amount} ${currency}<br />الرسوم: ${fee} ${currency}<br />الحالة: ${status}<br />العنوان: ${address}<br />معرّف المعاملة: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> تحية<br> فريق${api_name} </p></div>",
	...
	"de": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>Sie haben einen ${currency} Auszahlungsantrag von ${amount} an die Adresse ${address} gemacht<br /><br />Betrag: ${amount}<br />Gebühr: ${fee} ${currency}<br />Adresse: ${address}<br />Network: ${network}<br /><br />Um diese Auszahlung zu bestätigen, klicken Sie bitte auf die Taste unten.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Bestätigen</Button></a></div><p>Wenn diese Anfrage fälschlicherweise gestellt wurde, können Sie sie getrost ignorieren; es werden keine Änderungen an Ihrem Konto vorgenommen.</p><p>Anfrage initiiert von: ${ip}</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p>Sehr geehrte/r ${name} </p><p> Sie haben eine neue Einzahlung von ${amount} ${currency} in Ihrer ${api_name} Geldbörse ausstehend. Bitte warten Sie, bis die Transaktion bestätigt ist und Ihr Guthaben in Ihrer Geldbörse verfügbar ist. Ihre Transaktion erfordert ${confirmation} Bestätigung(en) in der Blockchain.</p><p>Betrag: ${amount} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Network: ${network}<br />Gebühr: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "${currency} Einzahlung pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> Sehr geehrte/r ${name} </p><p> Ihre ${currency} Anzahlung für ${amount} ${currency} ist bestätigt und abgeschlossen und steht in Ihrer ${currency} Geldbörse zur Verfügung.</p><p>Betrag: ${amount} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Network: ${network}<br />Gebühr: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "${currency} Einzahlung completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>Sie haben eine Auszahlungsanforderung für ${amount} ${currency} gemacht. Ihr Auszahlungsstatus ist ausstehend und wird in Kürze bearbeitet.</p><p>Betrag: ${amount} ${currency}<br />Gebühr: ${fee} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
			"title": "${currency} Abbuchung request"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> Sehr geehrte/r ${name} </p><p>Ihr Auszahlungsantrag für ${amount} ${currency} wurde bearbeitet.</p><p>Betrag: ${amount} ${currency}<br />Gebühr: ${fee} ${currency}<br />Status: ${status}<br />Adresse: ${address}<br />Transaktionsnummer: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Mit freundlichen Grüßen<br> ${api_name} team </p></div>",
	...
	"es": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>You have made a ${currency} withdrawal request of ${amount} to ${address}<br /><br />Cantidad: ${amount}<br />Fee: ${fee} ${currency}<br />Dirección: ${address}<br />Network: ${network}<br /><br />In order to confirm this withdrawal, please click the button below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm</Button></a></div><p>Si esta solicitud se hizo por error, es seguro ignorarla; no se harán cambios en su cuenta.</p><p>Solicitud iniciada de: ${ip}</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
		...
		"BANK_VERIFIED": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> Estimado/ Estimada ${name} </p><p>Tiene un nuevo depósito por ${amount} ${currency} pendiente en su ${api_name} billetera. Por favor espere hasta que la transacción se confirme y sus fondos estarán disponibles en su billetera. Su transacción requiere ${confirmation} confirmación(es) en la cadena de bloqueo.</p><p>Cantidad: ${amount} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />Network: ${network}<br />Tarifa: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "${currency} Depósito pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> Estimado/ Estimada ${name} </p><p> Su ${currency} depósito por ${amount} ${currency} está confirmado y completado y está disponible en su ${currency} billetera.</p><p>Cantidad: ${amount} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />Network: ${network}<br />Tarifa: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "${currency} Depósito completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>Usted hizo una solicitud de retiro de ${amount} ${currency}. El estado de su retirada está pendiente y se procesará en breve.</p><p>Cantidad: ${amount} ${currency}<br />Tarifa: ${fee} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacción: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
			"title": "${currency} Retirada request"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> Estimado/ Estimada ${name} </p><p>Su solicitud de retirada de ${amount} ${currency} se está procesando.</p><p>Cantidad: ${amount} ${currency}<br />Tarifa: ${fee} ${currency}<br />Estado: ${status}<br />Dirección: ${address}<br />ID de transacciónID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Saludos<br>Equipo de  ${api_name}  </p></div>",
	...
	"fa": {
		...
		"INVALID_ADDRESS": {
			"html": "<div><p>  کاربر عزیز${name} </p><p>Your ${currency} withdrawal for ${amount} was being sent to an invalid address and is rejected.</p><p>آدرس: ${address}</p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Invalid Withdrawal Address"
		},
		"USER_DEACTIVATED": {
			"html": "<div><p>Your account ${email} has been deactivated. You will not be able to use your account until it is activated by the exchange admin.</p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Account ${type}"
		},
		"USER_ACTIVATED": {
			"html": "<div><p>Your account ${email} has been activated. You are now able to use your account.</p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Account ${type}"
		},
		"DISCOUNT_UPDATE": {
			"html": "<div><p>  کاربر عزیز${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p>  کاربر عزیز${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p>  کاربر عزیز${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> کاربر عزیز${name}} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> کاربر عزیز${name} </p><p> شما یک واریز جدید به مبلغ ${amount} ${currency} دارید که در حال انتظار برای واریز به کیف پول ${api_name} می باشد. لطفا تا تایید تراکنش خود و مشاهده مبلغ در کیف پول خود تامل فرمایید. تراکنش شما حداقل به  ${confirmation} تایید بر روی شبکه بلاکچین نیاز دارد.` </p><p>مبلغ: ${amount} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />Network: ${network}<br />کارمزد: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "${currency} واریز pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> کاربر عزیز${name}} </p><p> واریز ${currency} شما به میزان ${amount} ${currency}  تکمیل و تایید شده و هم اکنون در کیف پول ${currency}  شما قابل استفاده است.</p><p>مبلغ: ${amount} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />Network: ${network}<br />کارمزد: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "${currency} واریز completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> کاربر عزیز${name} </p><p>شما درخواست برداشت به مبلغ ${amount} ${currency}.و به آدرس ${address} ثبت کرده اید. درخواست برداشت شما درحال انتظار بوده و به زودی مورد پردازش قرار خواهد گرفت.</p><p>مبلغ: ${amount} ${currency}<br />کارمزد: ${fee} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> با تشکر<br> ${api_name} تیم </p></div>",
			"title": "${currency} برداشت request"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> کاربر عزیز${name} </p><p>درخواست برداشت شما به مبلغ ${amount} ${currency} درحال انجام و ارسال به آدرس ${address} می باشد.<br />کارمزد: ${fee} ${currency}<br />Status: ${status}<br />آدرس: ${address}<br />شماره پیگیری تراکنش: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Regards<br> ${api_name} team </p></div>",
			"title": "${currency} برداشت completed"
		}
	},
	"fr": {
		"LOGIN": {
			"html": "<div> <p> Bonjour ${name} </p> <p> We have recorded a login to your account with the following details </p> <div> <div>Heure: ${time}</div> <div>Pays ${country}</div> <div>Adresse IP: ${ip}</div> </div> <p> If this was not you, please change your password, set up two-factor authentication, and contact us immediately. </p> <p> Bien cordialement<br> ${api_name} team </p> </div>",
			"title": "Login"
		},
		"SIGNUP": {
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> Bonjour ${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> Bonjour ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> Bonjour ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> Bonjour ${name} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> Bonjour ${name} </p><p> Vous avez un nouveau dépôt pour ${amount} ${currency} en attente dans votre portefeuille ${api_name} . Veuillez attendre que la transaction soit confirmée et vos fonds seront disponible dans votre portefeuille. Votre transaction nécessite ${confirmation} confirmation(s) sur la blockchain.</p><p>Montant: ${amount} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Network: ${network}<br />Frais: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "${currency} Deposit pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> Bonjour ${name} </p><p> Votre ${currency} dépôt pour ${amount} ${currency} est confirmé et complété et il est disponible dans votre portefeuille ${currency}.</p><p>Montant: ${amount} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Network: ${network}<br />Frais: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "${currency} Deposit completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> Bonjour ${name} </p><p>Vous avez fait une demande de retrait de ${amount} ${currency}. Le statut de votre retrait est en attente et sera traité sous peu.</p><p>Montant: ${amount} ${currency}<br />Frais: ${fee} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "${currency} withdrawal pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> Bonjour ${name} </p><p>Votre demande de retrait de ${amount} ${currency} est traité.</p><p>Montant: ${amount} ${currency}<br />Frais: ${fee} ${currency}<br />Status: ${status}<br />Addresse: ${address}<br />Identité de la transaction: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Bien cordialement<br> ${api_name} team </p></div>",
			"title": "${currency} withdrawal completed"
		}
	},
	"id": {
		"LOGIN": {
			"html": "<div> <p> Kepada ${name} </p> <p> We have recorded a login to your account with the following details </p> <div> <div>Waktu: ${time}</div> <div>Negara: ${country}</div> <div>Alamat IP: ${ip}</div> </div> <p> If this was not you, please change your password, set up two-factor authentication, and contact us immediately. </p> <p> Salam<br> ${api_name} tim </p> </div>",
			"title": "Masuk"
		},
		"SIGNUP": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> Kepada ${name} </p><p>Anda telah membuat permintaan penarikan ${currency} dengan jumlah ${amount} ke ${address}<br /><br />Jumlah: ${amount}<br />Biaya: ${fee} ${currency}<br />Alamat: ${address}<br />Network: ${network}<br /><br />Untuk konfirmasi penarikan ini, silakan klik tombol di bawah.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Konfirmasi</Button></a></div><p>Jika Anda tidak ingin melanjutkan permintaan ini, silakan abaikan ini; tidak ada perubahan pada akun Anda.</p><p>Permintaan dari: ${ip}</p><p> Salam<br> ${api_name} tim </p></div>",
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> Kepada ${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> Salam<br> ${api_name} tim </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> Kepada ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Salam<br> ${api_name} tim </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> Kepada ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Salam<br> ${api_name} tim </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> Kepada ${name} </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Salam<br> ${api_name} tim </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> Kepada ${name} </p><p> Ada deposit baru untuk ${amount} ${currency} yang sedang dalam proses di dompet ${api_name} Anda. Mohon ditunggu sampai transaksi dikonfirmasi dan dana Anda akan tersedia di dompet Anda. Please wait until the transaction is confirmed and your funds will be available in your wallet. Transaksi Anda memerlukan ${confirmation} konfirmasi dalam blockchain.</p><p>Jumlah: ${amount} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Network: ${network}<br />Biaya: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Salam<br> ${api_name} tim </p></div>",
			"title": "${currency} Deposito pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> Kepada ${name} </p><p> Deposit ${currency} Anda untukr ${amount} ${currency} telah dikonfirmasi dan berhasil dibuat dan tersedia di dompet ${currency} Anda.</p><p>Jumlah: ${amount} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Network: ${network}<br />Biaya: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Salam<br> ${api_name} tim </p></div>",
			"title": "${currency} Deposito completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> Kepada ${name} </p><p>Anda telah membuat permintaan penarikan untuk ${amount} ${currency}. ke alamat ${address}. Penarikan Anda sedang dalam proses dan akan segera diproses.</p><p>Jumlah: ${amount} ${currency}<br />Biaya: ${fee} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Salam<br> ${api_name} tim </p></div>",
			"title": "${currency} Penarikan pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p>Kepada ${name} </p><p>Permintaan penarikan Anda untuk ${amount} ${currency} telah diproses dan ditransfer ke alamat ${address}.</p><p>Jumlah: ${amount} ${currency}<br />Biaya: ${fee} ${currency}<br />Status: ${status}<br />Alamat: ${address}<br />ID Transaksi: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Salam<br> ${api_name} tim </p></div>",
			"title": "${currency} Penarikan completed"
		}
	},
	"ja": {
		"LOGIN": {
			"html": "<div> <p> ${name}様 </p> <p> お客様のアカウントが以下の情報からログインされました。 </p> <div> <div>Time: ${time}</div> <div>国家： ${country}</div> <div>IPアドレス： ${ip}</div> </div> <p> 本人のログインではない場合、${api_name}でパスワードの変更および二段階認証を設定し、即時にこのメールにてご返信ください。 </p> <p> 敬具<br> ${api_name} チーム </p> </div>",
			"title": "ログイン"
		},
		"SIGNUP": {
			"html": "<div> <p> ${name}様 </p> <p> You need to confirm your email account by clicking the button below.<br> If you have any questions feel free to contact us simply by replying to this email.</p><p>Please click on the button below to proceed with your registration.</p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">出金承認</Button></a></div><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "会員登録"
		},
		"WELCOME": {
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> ${name}様 </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> ${name}様 </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> ${name}様 </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> ${name}様 </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> ${name}様 </p><p> 現在、お客様の${api_name}ウォレットへの${amount} ${currency}入金は保留中です。取引が承認されるまでお待ちください お客様の取引には、ブロックチェーン上で${confirmation}個の承認が必要です。</p><p>金額: ${amount} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />Network: ${network}<br />手数料: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "${currency} Deposit pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> ${name}様 </p><p> お客様の${amount} ${currency}入金が完了しました。お客様の${currency}ウォレットから確認および利用が可能です。 </p><p>金額: ${amount} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />Network: ${network}<br />手数料: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "${currency} Deposit completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> ${name}様 </p><p>お客様のアドレス${address}に${amount} ${currency}出金がリクエストされました。現在の取引状態は保留中ですが、まもなく完了する予定です。</p><p>金額: ${amount} ${currency}<br />手数料: ${fee} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "${currency} withdrawal pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> ${name}様 </p><p>お客様の${amount} ${currency}出金が完了し、アドレス${address}に振り込みされました。</p><p>金額: ${amount} ${currency}<br />手数料: ${fee} ${currency}<br />取引状態: ${status}<br />アドレス: ${address}<br />取引ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 敬具<br> ${api_name} チーム </p></div>",
			"title": "${currency} withdrawal completed"
		}
	},
	"ko": {
		...
		"SUSPICIOUS_DEPOSIT": {
			"html": "<div><h3>Suspicious Deposit</h3><div>The client with email ${email} has received a ${currency} deposit that is suspicious.<br />거래 ID 확인: ${txid}<h4>Transaction data:</h4><div>${data}</div></div></div>",
			"title": "이중 지출"
		},
		"INVALID_ADDRESS": {
			"html": "<div><p> ${name}님 </p><p>Your ${currency} withdrawal for ${amount} was being sent to an invalid address and is rejected.</p><p>주소: ${address}</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Invalid Withdrawal Address"
		},
		"USER_DEACTIVATED": {
			"html": "<div><p>Your account ${email} has been deactivated. You will not be able to use your account until it is activated by the exchange admin.</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Account ${type}"
		},
		"USER_ACTIVATED": {
			"html": "<div><p>Your account ${email} has been activated. You are now able to use your account.</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Account ${type}"
		},
		"DISCOUNT_UPDATE": {
			"html": "<div><p> ${name}님 </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> ${name}님 </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> ${name}님 </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> ${name}님 </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> ${name}님 </p><p> 회원님의 ${api_name} 지갑으로 ${amount} ${currency} 입금이 진행 중입니다. 거래가 승인되고 지갑에 자금이 입금될 때까지 기다려주십시오. 회원님의 거래에는 비트코인 블록체인 상 ${confirmation} 개의 승인이 요구됩니다. </p><p>금액: ${amount} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />Network: ${network}<br />수수료: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "${currency} 입금 pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> ${name}님} </p><p> 회원님의 ${amount} ${currency} 입금이 완료되었습니다. 회원님의 ${currency} 지갑에서 확인 및 이용하실 수 있습니다.</p><p>금액: ${amount} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />Network: ${network}<br />수수료: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "${currency} 입금 completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> ${name}님 </p><p>회원님의 ${amount} ${currency} 출금이 요청되었습니다. 출금 대기 중이며, 곧 완료될 예정입니다.</p><p>금액: ${amount} ${currency}<br />수수료: ${fee} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "${currency} 출금 pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> ${name}님 </p><p>회원님의 ${amount} ${currency}를 출금이 완료되어 회원님의 계좌로 이체되었습니다.</p><p>금액: ${amount} ${currency}<br />수수료: ${fee} ${currency}<br />입금 상태: ${status}<br />주소: ${address}<br />거래 ID 확인: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 이용해 주셔서 감사합니다.<br> ${api_name} 팀 </p></div>",
			"title": "${currency} 출금 completed"
		}
	},
	"pt": {
		"LOGIN": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> Dear ${name} </p><p>Você fez uma solicitação de saque de ${currency} de ${amount} para ${address}<br /><br />Quantia: ${amount}<br />Taxa: ${fee} ${currency}<br />Endereço: ${address}<br />Network: ${network}<br /><br />Para confirmar este saque, por favor clique no botão abaixo.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirmar</Button></a></div><p>Se esta solicitação foi feita por engano, gentileza ignorá-la; nenhuma alteração será feita em sua conta.</p><p>Solicitação iniciada em: ${ip}</p><p> Saudações<br> ${api_name} equipe </p></div>",
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> Dear ${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> Saudações<br> ${api_name} equipe </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> Dear ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Saudações<br> ${api_name} equipe </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> Dear ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Saudações<br> ${api_name} equipe </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
		...
		"WITHDRAWAL_COMPLETED": {
			...
			"title": "${currency} Saque completed"
		}
	},
	"vi": {
		"LOGIN": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> ${name} thân mến </p><p>Quý khách vừa thực hiện yêu cầu rút ${currency} với số lượng ${amount} tới ${address}<br /><br />Số lượng: ${amount}<br />Phí: ${fee} ${currency}<br />Địa chỉ: ${address}<br />Network: ${network}<br /><br />Để xác nhận yêu cầu rút tiền này, vui lòng nhấn vào nút bên dưới.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Xác nhận</Button></a></div><p>Trong trường hợp không mong muốn thay đổi nêu trên, hãy bỏ qua tin nhắn này. Chúng tôi sẽ không thực hiện bất kỳ thay đổi nào trên tài khoản của quý khách.</p><p>Yêu cầu được gửi đến từ: ${ip}</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
		...
		"DISCOUNT_UPDATE": {
			"html": "<div><p> ${name} thân mến </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> ${name} thân mến </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> ${name} thân mến </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "Password Changed"
		},
		"CHANGE_PASSWORD": {
			"html": "<div><p> ${name} thân mến </p><p>You have made a request to change the password for your account.<br />To confirm your password changed, click on the link below.<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">Confirm Change My Password</Button></a></div><p>If this request was made in error, it is safe to ignore it; no changes will be made to your account.</p><p>Request initiated from: ${ip}</p><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "Change Password Confirmation"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> ${name} thân mến </p><p> Một giao dịch nạp ${amount} ${currency} vào địa chỉ ví ${api_name} của quý khách đang chờ được xử lý. Vui lòng chờ đợi cho đến khi giao dịch được xác nhận và tiền được nạp vào ví của quý khách. Yêu cầu giao dịch của quý khách cần nhận được ${confirmation} kết quả đồng thuận trên blockchain.</p><p>Số lượng: ${amount} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Network: ${network}<br />Phí: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "${currency} Nạp tiền pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> ${name} thân mến </p><p> Giao dịch nạp ${currency} với số lượng ${amount} ${currency} đã được xác nhận và hoàn tất thành công. Số tiền đã được cập nhập vào ví ${currency} của quý khách.</p><p>Số lượng: ${amount} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Network: ${network}<br />Phí: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "${currency} Nạp tiền completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> ${name} thân mến </p><p>Quý khách vừa thực hiện yêu cầu rút ${amount} ${currency} tới địa chỉ ${address}. Yêu cầu rút tiền của quý khách đang ở trạng thái chờ xử lý và sẽ sớm được thực hiện.</p><p>Số lượng: ${amount} ${currency}<br />Phí: ${fee} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul<p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "${currency} Rút tiền pending"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> ${name} thân mến </p><p>Yêu cầu rút ${amount} ${currency} của quý khách đã được thực hiện và số tiền tương ứng đã được chuyển thành công tới địa chỉ ${address}.</p><p>Số lượng: ${amount} ${currency}<br />Phí: ${fee} ${currency}<br />Trạng thái: ${status}<br />Địa chỉ: ${address}<br />ID giao dịch: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> Cảm ơn quý khách vì đã sử dụng dịch vụ của chúng tôi. <br> Đội ngũ  ${api_name}  </p></div>",
			"title": "${currency} Rút tiền completed"
		}
	},
	"zh": {
		"LOGIN": {
		...
		"WITHDRAWAL_REQUEST": {
			"html": "<div><p> ${name} </p><p>你已提出${currency}提款申请，金额为${amount}至${address}。<br /><br />金额: ${amount}<br />手续费: ${fee} ${currency}<br />地址: ${address}<br />Network: ${network}<br /><br />为完成此次提款申请，请点击下方按钮。<br /></p><div style=\"padding-top: 10px; margin-bottom: 10px;\"><a href=\"${link}\" target=\"_blank\"><Button style=\"cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;\">确认</Button></a></div><p>如果该请求是错误的，请忽略，并且你的账号不会有任何改变</p><p>提出申请的出处: ${ip}</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
		...
		"SUSPICIOUS_DEPOSIT": {
			"html": "<div><h3>Suspicious Deposit</h3><div>The client with email ${email} has received a ${currency} deposit that is suspicious.<br />交易 ID: ${txid}<h4>Transaction data:</h4><div>${data}</div></div></div>",
			"title": "可疑存款"
		},
		"INVALID_ADDRESS": {
			"html": "<div><p> ${name} </p><p>Your ${currency} withdrawal for ${amount} was being sent to an invalid address and is rejected.</p><p>地址: ${address}</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Invalid Withdrawal Address"
		},
		"USER_DEACTIVATED": {
			"html": "<div><p>Your account ${email} has been deactivated. You will not be able to use your account until it is activated by the exchange admin.</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Account ${type}"
		},
		"USER_ACTIVATED": {
			"html": "<div><p>Your account ${email} has been activated. You are now able to use your account.</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Account ${type}"
		},
		"DISCOUNT_UPDATE": {
			"html": "<div><p> ${name} </p><p>Your discount rate has been changed to ${rate}%. This rate will be applied to your order fees.</p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Discount Rate Change"
		},
		"BANK_VERIFIED": {
			"html": "<div><p> ${name} </p><p>A pending bank account has been verified. Your valid account can now be used for exchange operations requiring a bank account.</p><div><strong>Verified Bank Accounts:</strong>${list_detail_bank_account}</div><p><a href=\"${link_verification}\">To view your current bank accounts, please visit the exchange's Verification Tab</a></p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Bank Verified"
		},
		"USER_ID_VERIFICATION_REJECT": {
		...
		"PASSWORD_CHANGED": {
			"html": "<div><p> ${name} </p><p>This email confirms that you recently changed the password for your account. No further action is required.<br />If you did not authorize this change please contact us immediately.<br /></p><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "Password Changed"
		},
		"DEPOSIT_PENDING": {
			"html": "<div><div><p> ${name} </p><p> 你的${api_name}钱包中有一笔${amount} ${currency}的新存款正在等待处理，待交易被确认后，资金才会进入到你的钱包中，该交易需要在区块链上进行${confirmation}确认。</p><p>金额: ${amount} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />Network: ${network}<br />手续费: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "${currency} 充值 pending"
		},
		"DEPOSIT_COMPLETED": {
			"html": "<div><div><p> ${name}  </p><p> 你的${amount} ${currency}存款已完成。你的${currency}可在钱包中进行确认。</p><p>金额: ${amount} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />Network: ${network}<br />手续费: ${fee} ${currency}<br />Description: ${description}<br /><ul>${explorers}</ul></div><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "${currency} 充值 completed"
		},
		"WITHDRAWAL_PENDING": {
			"html": "<div><p> ${name} </p><p>你向该地址${address}申请了${amount} ${currency}的提款请求，该提款地址正在等待处理并会很快得到处理。</p><p>金额: ${amount} ${currency}<br />手续费: ${fee} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "${currency} 提款 request"
		},
		"WITHDRAWAL_COMPLETED": {
			"html": "<div><p> ${name} </p><p>你的${amount} ${currency}已向该地址${address}提款完毕。</p><p>金额: ${amount} ${currency}<br />手续费: ${fee} ${currency}<br />状态: ${status}<br />地址: ${address}<br />交易 ID: ${transaction_id}<br />Network: ${network}<br />Description: ${description}<br /><ul>${explorers}</ul><p> 感谢你的使用<br> ${api_name} 团队 </p></div>",
			"title": "${currency} 提款 completed"
		}
	},
	"template": {
		"link": "text-decoration: none; color: #000;",
		"logo": "text-align: center; margin-bottom: 1.5rem; background-color: #333; padding-bottom: 1rem; padding-top: 1rem;",
		"title": "padding-top: 20px; padding-bottom: 20px;",
		"button": "cursor: pointer; background-color: #333333; color: white; border: none; padding: 1rem; text-transform: uppercase; cursor: pointer !important; font-size: 14px; min-width: 11rem;",
		"footer": "padding: 1rem; padding-top: 1rem; padding-bottom: 3rem; background-color: transparent; color: #000 !important; font-size: 12px",
		"header": "padding-top: 3rem; padding-bottom: 1rem; background-color: #ffffff; text-align: center",
		"wrapper": "align-items:center; max-width: 45rem; margin: 0 auto;",
		"container": "background-color: #fff; color: #000 !important; padding: 5rem; padding-top: 30px; padding-bottom: 15px;",
		"link_blue": "text-decoration: none; color: #35b2e8 !important;",
		"poweredby": "margin-top: 0.5rem",
		"box_shadow": "-webkit-box-shadow: 0px 4px 10px 2px rgba(204,204,204,1) !important; -moz-box-shadow: 0px 4px 10px 2px rgba(204,204,204,1) !important; box-shadow: 0px 4px 10px 2px rgba(204,204,204,1) !important;",
		"header_icon": "width: 60px",
		"buttonWrapper": "padding-top: 10px; margin-bottom: 10px;",
