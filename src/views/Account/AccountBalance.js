				...
				<tbody>
					...
					<tr style={{borderTop:'3px solid #81868a'}}>
						...
						<td>
							<div style={{color:'#2aabe4'}} className='row'>
								<div className='pl-3'>{numbro(fiat_available).format(FIAT_FORMAT)}</div>
								<div style={{backgroundColor:'#2aabe4'}} className='balance ml-1'></div>
							</div>
						</td>
					 	<td >
							<div style={{color:'#f15a25'}} className='row'>
								<div className='pl-3'>{numbro(btc_available).format(BTC_FORMAT)}</div>
					...
					<tr style={{borderTop:'2px solid #81868a'}}>
						...
						<td >
							<div style={{color:'#00a99e'}} className='row'>
								<div className='pl-3'>{numbro(fiat_trade).format(FIAT_FORMAT)}</div>
								<div style={{backgroundColor:'#00a99e'}} className='balance ml-1'></div>
							</div>
						</td>
						<td>
							<div style={{color:'#fbb03b'}} className='row'>
								<div className='pl-3'>{numbro(btc_trade).format(BTC_FORMAT)}</div>
								<div style={{backgroundColor:'#fbb03b'}} className='balance ml-1'></div>
							</div>
						</td>
						<td>Total in USD: {numbro(math.chain(btc_trade).multiply(price).add(fiat_trade).done()).format(FIAT_FORMAT)}</td>
					</tr>
					<tr style={{backgroundColor:'#e1e2e6'}}>
						...
						<td  className='accountActive '>
							<div>Total Asset in USD:${numbro(math.chain(btc_balance).multiply(price).add(fiat_balance).done()).format(FIAT_FORMAT)}</div>
						</td>
					</tr>
				</tbody>
			</table>
