...
const VerificationNotification = ({ data: { type, onClick } }) => {
  ...
  return (
    <NotificationWraper {...notificationProps} className="notification_verification">
      <NotificationContent>
        {type === 'skip' ? STRINGS.VERIFICATION_NOTIFICATION_SKIP_TEXT : STRINGS.VERIFICATION_NOTIFICATION_SUCCESS_TEXT}
      </NotificationContent>
      <Button label={STRINGS.VERIFICATION_NOTIFICATION_BUTTON} onClick={onClick} />
    </NotificationWraper>
  );
}
