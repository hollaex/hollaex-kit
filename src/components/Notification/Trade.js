...
const TradeDisplay = ({ side, data }) => {
  const actionText = side === 'sell' ? STRINGS.ORDER_SOLD : STRINGS.ORDER_BOUGHT;
  const resultText = side === 'sell' ? STRINGS.ORDER_RECEIVED : STRINGS.ORDER_SPENT;
  const { btc, fiat, average } = calculateValues(data);
  return (
    <NotificationContent>
      <InformationRow
        label={actionText}
        value={STRINGS.formatString(STRINGS.BTC_PRICE_FORMAT, btc, STRINGS.BTC_SHORTNAME)}
      />
      <InformationRow
          label={STRINGS.ORDER_AVERAGE_PRICE}
          value={STRINGS.formatString(STRINGS.FIAT_PRICE_FORMAT, average, STRINGS.FIAT_SHORTNAME)}
        />
      <InformationRow
        label={resultText}
        value={STRINGS.formatString(STRINGS.FIAT_PRICE_FORMAT, fiat, STRINGS.FIAT_SHORTNAME)}
      />
    </NotificationContent>
  );
}
