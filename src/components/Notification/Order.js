...
const generateRows = (type, order) => {
  ...
  if (type === 'order_added' && order.filled === 0) {
    rows.push({
      label: STRINGS.SIZE,
      value: STRINGS.formatString(STRINGS.BTC_PRICE_FORMAT, formatBtcAmount(order.size), STRINGS.BTC_SHORTNAME),
    });
    rows.push({
      label: STRINGS.PRICE,
      value: STRINGS.formatString(STRINGS.FIAT_PRICE_FORMAT, formatFiatAmount(order.price), STRINGS.FIAT_SHORTNAME),
    });
  } else {
    ...
    rows.push({
      ...
      value: STRINGS.formatString(STRINGS.BTC_PRICE_FORMAT, formatBtcAmount(order.filled), STRINGS.BTC_SHORTNAME),
    });
    rows.push({
      label: STRINGS.PRICE,
      value: STRINGS.formatString(STRINGS.FIAT_PRICE_FORMAT, formatFiatAmount(order.price), STRINGS.FIAT_SHORTNAME),
    ...
    rows.push({
      ...
      value: STRINGS.formatString(STRINGS.FIAT_PRICE_FORMAT, formatFiatAmount(orderValue), STRINGS.FIAT_SHORTNAME),
    });

    if (type === 'order_added') {
      rows.push({
        label: STRINGS.REMAINING,
        value: STRINGS.formatString(STRINGS.BTC_PRICE_FORMAT, formatBtcAmount(remaining), STRINGS.BTC_SHORTNAME),
      });
    }
  }
