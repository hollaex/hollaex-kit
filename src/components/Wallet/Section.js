...
const Section = ({ symbol = fiatSymbol, balance, orders, price }) => {
  ...
  const amountFormat = symbol === fiatSymbol ? STRINGS.FIAT_PRICE_FORMAT : STRINGS.BTC_PRICE_FORMAT;
    ...
    <div className="wallet_section-content-wrapper">
      <div className="wallet_section-content d-flex flex-column">
        <div className="d-flex flex-column">
          ...
          <div>{STRINGS.formatString(amountFormat, formatToCurrency(total), currencySymbol)}</div>
        </div>
        {ordersOfSymbol > 0 &&
          <TextHolders
        ...
        <div className="d-flex flex-column">
          ...
          <div>{STRINGS.formatString(amountFormat, formatToCurrency(available), currencySymbol)}</div>
        </div>
      </div>
    </div>
  );
