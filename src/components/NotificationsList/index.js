...
const NotificationItem = (props) => {
  ...
  return (
    <div className="notifications_list-itme-container">
      ...
      {data && <div className="notifications_list-item-text">{JSON.stringify(data)}</div>}
      {timestamp && <div className="notifications_list-item-timestamp">{formatTimestamp(timestamp)}</div>}
    </div>
  )
}
