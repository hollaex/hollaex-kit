        ...
        <tr key={index}>
            <td className="account-limits-coin" rowSpan={2}>
                <div className='d-flex align-items-center'>
                    <CurrencyBall name={STRINGS[`${pair_base.toUpperCase()}_SHORTNAME`]} symbol={pair_base} />
                    <div className="ml-2">
                        {`${STRINGS[`${pair_base.toUpperCase()}_FULLNAME`]} / ${STRINGS[`${pair_2.toUpperCase()}_FULLNAME`]}`}
                    </div>
                </div>
            </td>
            <td className="account-limits-status">{STRINGS.SUMMARY.MAKER}:</td>
            <td className="account-limits-value">{formatPercentage(data.maker_fees)}</td>
        </tr>
    );
}
...
const getTakerRow = (data, pair, index) => {
    return (
        <tr key={`${index}_1`}>
            <td className="account-limits-status">{STRINGS.SUMMARY.TAKER}:</td>
            <td className="account-limits-value">{formatPercentage(data.taker_fees)}</td>
        </tr>
    );
}
