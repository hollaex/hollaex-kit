...
import { AppBar, TabController, CheckTitle, Dialog, Loader, Logout, Notification } from '../../components';
import { ICONS } from '../../config/constants';
import STRINGS from '../../config/localizedStrings';
import { requestSmsCode, verifySmsCode, verifyBankData, getUserData } from '../../actions/verificationActions';
...
const CONTENT_CLASS = 'd-flex justify-content-center align-items-center f-1 flex-column verification_content-wrapper';
...
class Verification extends Component {
  ...
  calculateActiveTab = ({ bank_account, address, phone_number, id_data, full_name }) => {
    if (!bank_account.card_number && !full_name) {
      return 0;
  ...
  updateTabs = (user = {}, activeLanguage = this.props.activeLanguage, activeTab = this.state.activeTab) => {
    if (activeTab === -1) {
      return;
    }
    ...
    const tabs = [
      {
        title: (
          <CheckTitle
            ...
            icon={activeTab === 0 ? ICONS.VERIFICATION_BANK : ICONS.VERIFICATION_BANK_INACTIVE}
          />
        ),
        content: activeTab === 0 && (
      ...
      },
      {
        title: (
          <CheckTitle
            ...
            icon={activeTab === 1 ? ICONS.VERIFICATION_ID : ICONS.VERIFICATION_ID_INACTIVE}
          />
        ),
        content: activeTab === 1 && (
          <div className={CONTENT_CLASS}>
      ...
      },
      {
        title: (
          <CheckTitle
            ...
            icon={activeTab === 2 ? ICONS.VERIFICATION_MOBILE : ICONS.VERIFICATION_MOBILE_INACTIVE}
          />
        ),
        content: activeTab === 2 && (
          <div className={CONTENT_CLASS}>
      ...
      },
      {
        title: (
          <CheckTitle
            ...
            icon={activeTab === 3 ? ICONS.VERIFICATION_DOC : ICONS.VERIFICATION_DOC_INACTIVE}
          />
        ),
        content: activeTab === 3 && (
          <div className={CONTENT_CLASS}>
            <DocumentsVerification
              ...
              initialValues={{ type: user.nationality === 'IR' ? 'id' : 'passport'}}
              moveToNextStep={this.goNextTab}
              skip={this.skip}
              activeLanguage={activeLanguage}
              openContactForm={this.openContactForm}
  ...
  render() {
    ...
    if (activeTab === -1 && tabs.length > 0) {
      return <div className="app_container"><Loader /></div>;
    }
    ...
    return (
      <div className={classnames('app_container', fontClass, languageClasses[0])}>
        <AppBar
          isHome={true}
          token={token}
          rightChildren={<Logout className="bar-logout" onLogout={this.onLogout} />}
        />
