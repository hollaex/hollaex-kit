...
class BankVerification extends Component {
  ...
  generateFormFields = (isIranianAccount = false) => {
    const formFields = {
      isIranianAccount: {
        type: 'select',
        label: STRINGS.USER_VERIFICATION.BANK_ACCOUNT_FORM.FORM_FIELDS.IRANIAN_ACCOUNT_LABEL,
        placeholder: STRINGS.USER_VERIFICATION.BANK_ACCOUNT_FORM.FORM_FIELDS.IRANIAN_ACCOUNT_LABEL,
        defaultValue: true,
        options: [
    ...
    if (isIranianAccount) {
      ...
      formFields.card_number = {
        ...
        placeholder: STRINGS.USER_VERIFICATION.BANK_ACCOUNT_FORM.FORM_FIELDS.CARD_NUMBER_PLACEHOLDER,
        validate: [required, onlyNumbers, exactLength(16, STRINGS.USER_VERIFICATION.BANK_ACCOUNT_FORM.VALIDATIONS.ACCOUNT_NUMBER)],
        maxLength: 16,
      };
    }
