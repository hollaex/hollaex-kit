...
export const findPath = (connections = [], start, end = BASE_CURRENCY, source_key = 'pair_base', target_key = 'pair_2') => {

  const connectionsFromStart = connections
    .filter(({ [source_key]: source }) => source === start)

  const connectionsFromStartToEnd = connectionsFromStart
    .filter(({ [target_key]: target }) => target === end)
