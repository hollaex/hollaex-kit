...
class GetSocketState extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (JSON.stringify(this.props.orderbooks) !== JSON.stringify(prevProps.orderbooks) ||
            JSON.stringify(this.props.pairsTrades) !== JSON.stringify(prevProps.pairsTrades)) {
            this.checkConnection();
        }
    }
    ...
    checkConnection = () => {
        if (!this.props.isDataReady) {
            ...
            let isReady = Object.keys(orderbooks).length &&
                orderbooks[pairTemp] &&
