...
const link_sections = (
    ...
    // if (is_custom) {
        const formFields = { section_1, section_2, section_3, section_4, section_5, ...fields };
        let count = 1;
                ...
                bottomLink: (
                    <div className="admin-link pointer">
                        <span onClick={() => addLink(key)}>
                            <PlusOutlined style={{ color: '#FFFFFF' }} />Add link
                        </span>
                    </div>
                )
            };
        ...
        formFields[`section_${count}`] = {
            ...
            bottomLink: (
                <Button block type="primary" onClick={() => addColumn(`section_${count}`)}>
                    Add column
                </Button>
            )
        };
...
class FooterConfig extends Component {
    ...
    handleToggle = (checked) => {
        this.setState({
            isCustom: checked,
            default_fields: link_sections(),
            custom_fields: link_sections(this.state.custom_fields, this.addLink, this.addColumn)
        });
    };
