...
const CreatePairSection = ({
    ...
    const checkExist = () => {
        const existPair = allPairs.filter((pair) => pair.name === `${formData.pair_base}-${formData.pair_2}`)
        if (!formData.pair_2) {
    ...
    return (
        <div className="add-pair-wrapper">
            ...
            <div className="coin-container">
                        ...
                        <div className="flex-container full-width">
                            ...
                            >
                                {coins.map((data, index) => {
                                    ...
                                    let fullname = typeof data === 'string' ? data : data.fullname;
                                    return (
                                        <Option key={index} value={symbol}>
                                            <img
                                                src={
                                                    getCoinSource(data, symbol)
                                                }
                                                alt="coins"
                                                className="coin-icon"
                                            />
                ...
                {!isExchangeWizard
                    ? <div className="footer-info">Can't find your asset? Create/add one <Link to="/admin/financials?tab=1&isAsset=true">here.</Link></div>
                    : null
                }
            </div>
            <div className="btn-wrapper">
                ...
                <Button className="green-btn" type="primary" onClick={checkExist} disabled={!formData.pair_base}>
                    Next
                </Button>
            </div>
        </div>
