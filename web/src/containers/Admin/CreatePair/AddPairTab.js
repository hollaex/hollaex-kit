...
const getCoinPairSelect = (currentPresetPair, pairBase, pair2, moveToStep) => {
    if (currentPresetPair.id) {
        return (<div className="coin-container">
            <div className="asset-pairs" onClick={() => moveToStep('pair-selection')}>
                <div>
            ...
            {currentPresetPair.issuer
                ? <div className="provider-display">Provider: {currentPresetPair.issuer}</div>
                : null
            }
        </div>)
...
const AddPairTab = ({
    ...
    const pairBase = allCoins.filter((data) => data.symbol === currentPresetPair.pair_base)[0] || {};
    const pair2 = allCoins.filter((data) => data.symbol === currentPresetPair.pair_2)[0] || {};
    ...
    const renderContent = () => {
        return (
            <div>
                ...
                <div className="add-pair-radio-wrapper">
                    <div>
                        Don't see your asset? <span className="anchor" onClick={handleNavigate}>Create a new pair</span>
                    </div>
                </div>
            </div>
        )
    ...
    return (
        <div className="add-pair-wrapper">
            ...
            {isExchangeWizard
                ?
                <div>
                    Trading pair are based on assets selected in the previous step. To
                    see more trading pairs <Link onClick={() => moveToParentStep(2)}>go back</Link> and add more assets.
                </div>
                : null
            }
            <Tab
            ...
            <div className="btn-wrapper">
                ...
                <Button className="green-btn" type="primary" onClick={() => moveToStep('preview')} disabled={!formData.pair_base}
                >
                    Next
                </Button>
            </div>
