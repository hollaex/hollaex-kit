...
const COLUMNS = (pairs, allCoins = [], user = {}, constants = {}, handlePreview = () => {}) => [
    {
        title: 'Market pairs',
        dataIndex: 'symbol',
        key: 'symbol',
    ...
    },
    {
        ...
        render: () => (
            <div>
                <div className={constants.features && !constants.features.pro_trade ? "custom-column-td column-divider bg-disable" : "custom-column-td column-divider"}>
                    <div className="d-flex align-items-center">
                        <Image
                            icon={STATIC_ICONS['CANDLES_LOGO']}
                            wrapperClassName="limit-status-icon mr-2"
                        />
                        Pro Trade
                    </div>
                </div>
                <div className={constants.features && !constants.features.quick_trade ? "custom-column-td bg-disable" : "custom-column-td"}>
                    <div className="d-flex align-items-center">
    ...
    },
    {
        ...
        render: () => (
            <div>
                <div className={constants.features && !constants.features.pro_trade ? "custom-column-td column-divider bg-disable" : "custom-column-td column-divider"}>
                    {constants.features && !constants.features.pro_trade
                        ?
                        <div className="d-flex align-items-center justify-content-center pad-10">
                            <img
                                src={STATIC_ICONS.PENDING_ORANGE_GEAR}
                                className="active-status-icon"
                                alt="active_icon"
                            />
                            <span className='disable-txt'>Custom</span>(<Link to="/admin/trade?tab=1" >Edit</Link>)
                        </div>
                        :
                        <div className="d-flex align-items-center justify-content-center pad-10">
                            <img
                ...
                <div className={constants.features && !constants.features.quick_trade ? "custom-column-td bg-disable" : "custom-column-td"}>
                    {constants.features && !constants.features.quick_trade
                        ?
                        <div className="d-flex align-items-center justify-content-center pad-10">
                            <img
                                src={STATIC_ICONS.PENDING_ORANGE_GEAR}
        ...
        if (
            ...
            if (tabParams.tab === '0') {
                if (tabParams.isViewDetails === 'true') {
                    const previewData = this.state.pairs.filter(data => tabParams.currentPair === data.name)
                        ...
                        {configurePairs.map((item, key) => {
                            ...
                            >
                                <div className="d-flex justify-content-between">
                                    <div className="d-flex align-items-center">
                                        <Link to={`/admin/trade?tab=0&isOpenPairModal=true&pairs=${pairs[0]}-${pairs[1]}`} className="mr-1">
                                            <IconToolTip
                                                type="settings"
                                    ...
                                    <div className="text-end">
                                        <div>(<Link to={`/admin/trade?tab=0&isOpenPairModal=true&pairs=${pairs[0]}-${pairs[1]}`} className="text-underline">Configure</Link>)</div>
                        ...
                        {pendingPairs.map((item, key) => {
                            ...
                            return <div key={key} className="box-content">
                                <div className="d-flex justify-content-between">
                                    ...
                                    <div className="text-end">
                                        <div>(<Link to={`/admin/trade?tab=0&isViewDetails=true&currentPair=${item.pair_base}-${item.pair_2}`} className="text-underline">View</Link>)</div>
                ...
                <div className="admin-pairs-container">
                    <Fragment>
                        ...
                        <div className="summary table-wrapper admin-tiers-wrapper">
                            <Table
                                columns={COLUMNS(allPairs, allCoins, user, constants, this.handlePreview)}
