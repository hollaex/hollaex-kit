...
const CustomizeEmailForm = ({ emailInfo, selectedLanguages, emailType, requestEmail }) => {
    ...
    const [selectedLanguage, setSelectedLanguage] = useState(selectedLanguages[0].value);
    ...
    const constructedData = useCallback(() => {
        let tempData = emailInfo[selectedLanguage];
        let temp1 = tempData[selectedEmail];
        if (temp1) {
            form.setFieldsValue({ format: JSON.stringify(temp1, undefined, 5) });
        } else {
            form.setFieldsValue({ format: JSON.stringify({}) });
        }
    }, [emailInfo, form, selectedEmail, selectedLanguage]);
    ...
    const handleSearch = (e) => {
        if (e.target.value) {
            const filterData = emailOptions.filter(data => data.includes(e.target.value.toUpperCase()));
    ...
    const handleSelectLanguage = (selectedLanguage) => {
        ...
        form.setFieldsValue({ format: JSON.stringify(emailInfo[selectedLanguage][emailType[0]], undefined, 5) });
    };
    ...
    const handleConfirmation = (formProps) => {
        const body = {
            email: {
                ...emailInfo,
                [formProps.language]: {
                    ...emailInfo[formProps.language],
                    [formProps.mailType]: JSON.parse(formProps.format),
                }
            }
        }
        ...
        if (modalType === 'reset-email') {
            return (
                <div className='confirm_modal_wrapper'>
                    ...
                    <div className='box-container'> Email to reset: {formData.mailType} </div>
                    <div className='mb-5'>
            ...
            return (
                <div className='confirm_modal_wrapper'>
                    ...
                    <div>After confirming the changes they will be saved and visible in future emails sent to your users.</div>
                    <div className='mb-4 mt-4'>
                        <b>Email preview</b>
                        <TextArea
                            rows={10}
                            value={formProps.format}
                        />
                    </div>
                    <div className='mb-4 mt-4'>Are you sure you want to save and apply the changes?</div>
                    <div className="btn-wrapper">
                        <Button
                            type="primary"
                            onClick={handleConfirmOpen}
    ...
    const initialValues = {
        ...
        format: emailInfo && JSON.stringify(emailInfo[selectedLanguages[0].value][emailType[0]], undefined, 5)
    }
    ...
    return (
        <div className='custom-email-wrapper'>
            ...
            >
                    ...
                    <div className='option-wrapper'>
                        <div className='d-flex justify-content-between'>
                            <div className="first-title">
                                Select email to customize
                            </div>
                            <CloseOutlined style={{ fontSize: '15px', height: '18px' }} onClick={handleOpenOption} />
                        </div>
                ...
                <div className={isDisable ? 'footer-section border-none' : 'footer-section'}>
                    <div className='d-flex align-items-end'>
                        {isDisable
                            ...
                            :
                            <div className='d-flex align-items-center' onClick={handleDisable}>
                                <div className='anchor'>
                                    EDIT EMAIL
                                </div>
                                <img
                        ...
                        {isEdit &&
                            <div className='small-text'>
                                <div>Last edit: <span>{lastEdit}</span></div>
                            </div>
                        }
                        {isReset &&
                            <div className='small-text anchor' onClick={() => handleConfirmOpen('reset-email')}>
