...
const Step2 = ({
    ...
    return (
        <Fragment>
            <div className="first-title">{activeTab === '0' ? 'HollaEx assets' : 'Other assets'}</div>
            <div className="title">Select a coin</div>
            <Input placeholder={'Search name or paste token contract address'} onChange={handleSearch} />
            <div className="sub-title">Asset:</div>
            <div className="coin-option-wrapper">
                {coins.filter(coin => (!coinKeys.includes(coin.symbol) && coin.verified))
                    .map((coin, index) => (
                        <div
                            key={index}
            ...
            <div className="create-new-link">
                ...
                <span className="anchor" onClick={handleCreateNew}>Create a brand new asset</span>
            </div>
        </Fragment>
    );
}
