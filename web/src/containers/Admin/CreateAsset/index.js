...
class CreateAsset extends Component {
    ...
    setCurrentPageAssets = (activeKey) => {
        ...
        let coins = this.props.coins.filter((val) => !coinKeys.includes(val.symbol) && val.verified && _toLower(val.issuer) === "hollaex");
        if (activeKey === '1') {
            coins = this.props.coins.filter((val) => !coinKeys.includes(val.symbol) && val.verified && _toLower(val.issuer) !== "hollaex");
        }
    ...
    handleFileChange = async (event, name) => {
        ...
        if (value && (value.split(".")[1].toUpperCase() === "JPG" ||
            ...
            message.warn("File format is invalid. Upload a validfile with jpeg, jpg and png extensions");
        }
    };
    ...
    handleSearch = (e) => {
        ...
        const filteredData = coins.filter((coin) => {
            return coin.symbol.toLowerCase().includes(searchValue)
