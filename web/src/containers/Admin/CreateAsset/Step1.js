...
const Step1 = ({
    ...
    const remainingCoins = coins.filter(coin => (!coinKeys.includes(coin.symbol) && coin.verified));
    const renderContent = () => {
        return (
            <div>
                <div className="existing-coin">
                    ...
                    {remainingCoins.length
                        ? <div>
                            <div className="asset-coin" onClick={() => handleScreenChange('step2')}>
                                <div>
                                    <img
                                        src={
                                            selectedCoinData && selectedCoinData.logo ? selectedCoinData.logo :
                                                STATIC_ICONS.COIN_ICONS[(selectedCoinData.symbol || '').toLowerCase()]
                                                    ? STATIC_ICONS.COIN_ICONS[(selectedCoinData.symbol || '').toLowerCase()]
                                        ...
                                        className="coin-icon"
                                    />
                                    {`${selectedCoinData.fullname} (${(selectedCoinData.symbol || '').toUpperCase()})`}
                                    {`${selectedCoinData.issuer ? ' - ' + selectedCoinData.issuer : ''}`}
                                </div>
                                <DownOutlined />
                            </div>
                            <div className="asset-coin-details">
                                <div className="coin-wrapper">
                                    <Coins
                                        ...
                                        color={selectedCoinData.meta ? selectedCoinData.meta.color : ''}
                                    />
                                </div>
                ...
                <div className="mt-3">
                    Don't see your asset? <span className="anchor" onClick={handleCreateNew}>Add your asset</span>
                </div>
