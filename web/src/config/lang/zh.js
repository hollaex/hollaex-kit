...
export default {
	...
	USER_VERIFICATION: {
		...
		ID_DOCUMENTS_FORM: {
			...
			INFORMATION: {
				...
				POR: {
					SECTION_1_TEXT_1:
						'为避免在验证账户时出现延误，请确认以下内容:',
					SECTION_1_TEXT_2:
						'姓名、地址、发行日期和发行者须清晰可见',
					SECTION_1_TEXT_3: '提交的居住证明不能超过三个月',
					SECTION_1_TEXT_4:
						'需要提交高像素（至少300dpi）的彩色照片或扫描图像',
	...
	ACCOUNT_SECURITY: {
		...
		OTP: {
			...
			CONTENT: {
				...
				MESSAGE_3:
					'如果在扫描QR码时遇到问题，可以手动输入以下QR码。',
				MESSAGE_4:
					'你可以安全地保存该QR码，以便以后更换或丢失手机时恢复2FA。',
				MESSAGE_5: '指南',
				INPUT: '输入一次性密码（OTP)',
				WARNING:
	...
	SIDES: [{ value: 'buy', label: '买入' }, { value: 'sell', label: '卖出' }],
	...
	WITHDRAWALS_MIN_VALUE_ERROR:
		'交易金额太小导致无法发送，请尝试更高的金额',
	WITHDRAWALS_MAX_VALUE_ERROR:
		'交易金额太大导致无法发送，请尝试较小的金额',
	...
	INVALID_USERNAME: '用户名长度必须在3~5个字符之间，只可包含小写、数字和下划线（_）',
	USERNAME_TAKEN: '该用户名已被注册，请重新输入。',
	USERNAME_LABEL: '用户名（聊天用名称）',
	USERNAME_PLACEHOLDER: '用户名',
	TAB_USERNAME: '用户名',
	USERNAME_WARNING:
		'用户名只可更改一次，请确保该用户名是最理想的。',
	...
	SUMMARY: {
		...
		JOIN_HAP:
			'加入HAP，每售出HollaEx Kit，就能获取10%的利润。', // new
		EARN_RUNNING_EXCHANGE:
			'经营自己的交易所，赚取被动收入。', // new
		XHT_WAVE_AUCTION: 'XHT波段拍卖数据', // new
		XHT_WAVE_DESC_1:
			'HollaEx 代币（XHT）的分发是通过波段拍卖完成的。', // new
		XHT_WAVE_DESC_2:
	...
	REFERRAL_LINK: {
		...
		APPLICATION_TXT:
			'若要成为HollaEx Kit经销商，请填写一份申请书。', // new
	...
	RESTART_TO_APPLY:
		'需要重新启动交易所才能应用这些更改。', // new
	TRIAL_EXCHANGE_MSG:
		'你使用的是{0}的试用版，它将在{1}天后失效。', // new
	EXPIRY_EXCHANGE_MSG:
