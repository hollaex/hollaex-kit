...
| `cancelAllOrders` | <ul><li>**opts**: Object with additional params</li><li>**opts.symbol**: (_optional_, _default_=`null`) HollaEx trading symbol e.g. `xht-usdt`</li></ul> (_optional_) | Cancel all open order. It can be filter by passing the symbol |
